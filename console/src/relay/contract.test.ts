...
describe('M1.1 wire compatibility', () => {
  ...
  test('accepts the relay captures projection and refuses a malformed record', () => {
    ...
    expect(parseRelayServerEvent({ ...base, captures: [{ ...open, files: Array.from({ length: 9 }, (_, index) => ({ ...file, file_id: `file-${index}` })) }] })).toBeNull()
    expect(parseRelayServerEvent({ ...base, captures: [{ ...open, files: [file, file] }] })).toBeNull()
