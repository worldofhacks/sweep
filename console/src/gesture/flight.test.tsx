...
import { FlightControls } from '../modules/gesture/FlightControls'
...
async function mount() {
  ...
  function Harness() {
    ...
    return <><video ref={producer.videoRef} /><FlightControls controller={control} /></>
  }
  render(<Harness />)
  await act(async () => {})
...
describe('Flight profile with two selected-capable nodes', () => {
  test('a mixed aircraft and robot selection blocks Flight buttons and recognition before creating an intent', async () => {
    ...
    const robot: RelayAircraftState = {
      ...drones()[0], drone_id: 11, device_class: 'ground_vehicle', unit: 1,
      // Even a misadvertised pulse capability and flight-like state cannot make a robot a flight target.
      adapter_capabilities: ['class:ground_vehicle', 'ground_drive', 'body_pulse_v1'],
    }
    emitState([1, 11], { drones: [...drones(), robot] })
    const flight = within(screen.getByRole('region', { name: 'Selected flight actions' }))
    expect(flight.getByText('Flight · D-01, G-01')).toBeInTheDocument()
    flight.getAllByRole('button').forEach((button) => expect(button).toBeDisabled())
    expect(flight.getAllByText('G-01 is a robot. Select aircraft only for the Flight profile.')).toHaveLength(5)
    hold('Victory', 650)
    expect(get().control.pendingRequest).toBeNull()
    expect(get().control.state.requests).toEqual([])
    act(() => expect(get().control.prepareIntent({
      name: 'body_pulse', args: { forward_mm_s: 250, duration_ms: 500 }, targets: [1, 11],
    })).toBeNull())
    act(() => expect(get().control.issueIntent({
      name: 'body_pulse', args: { forward_mm_s: -250, duration_ms: 500 }, targets: [11],
    })).toBeNull())
    expect(get().control.state.requests).toEqual([])
