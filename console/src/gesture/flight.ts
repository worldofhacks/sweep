...
export function flightActionBlockedReason(
  ...
  if (action.name === 'body_pulse') {
    ...
    if (unsupported !== undefined) return `${label(unsupported)} does not advertise ${BODY_PULSE_CAPABILITY}.`
    const grounded = state.selection.find((id) => !['airborne', 'hovering'].includes(state.aircraft[id].flight_state ?? ''))
    if (grounded !== undefined) return `${label(grounded)} must report airborne or hovering before a pulse.`
  }
  return null
}
