...
test('previews a target-free survey without accepting a fabricated class', async () => {
  const survey = { ...intent, args: { zone_id: 'lobby', mode: 'survey' as const } }
  const response = { ...previewResponse(), intent_id: survey.intent_id, preview: { ...searchPreview().preview, target_class: null, mode: 'survey' } }
  const client = new HttpSearchClient(
    { baseUrl: 'https://relay.example', token: 'test-token' },
    vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify(response))),
  )

  await expect(client.preview(survey)).resolves.toMatchObject({
    preview: { zone_id: 'lobby', target_class: null, mode: 'survey' },
  })
})
