...
export interface SearchClient { catalog(sessionId: string): Promise<SearchCatalog>; preview(intent: IntentV1): Promise<SearchPreview>; status(sessionId: string, intentId: string): Promise<SearchStatus>; acknowledge(sessionId: string, intentId: string, sightingId: string): Promise<SearchStatus> }
...
export class HttpSearchClient implements SearchClient {
  ...
  async preview(intent: IntentV1): Promise<SearchPreview> {
    ...
    if (!record(value) || value.session !== intent.session || value.intent_id !== intent.intent_id || !integer(value.t) || !integer(value.expires_at_ms) || !record(value.preview) || value.preview.zone_id !== args.zone_id || value.preview.target_class !== args.target_class || !Array.isArray(value.preview.allocations) || !value.preview.allocations.every(allocation)) throw new Error('The relay returned an invalid search preview.')
    return { session: value.session as string, intent_id: value.intent_id as string, t: value.t, expiresAt: value.expires_at_ms, preview: { zone_id: value.preview.zone_id as string, target_class: value.preview.target_class as string, allocations: value.preview.allocations as SearchPreview['preview']['allocations'] } }
  }
  async status(sessionId: string, intentId: string): Promise<SearchStatus> { return this.parseStatus(await this.request(sessionId, encodeURIComponent(intentId)), sessionId) }
  async acknowledge(sessionId: string, intentId: string, sightingId: string): Promise<SearchStatus> { return this.parseStatus(await this.request(sessionId, `${encodeURIComponent(intentId)}/findings/${encodeURIComponent(sightingId)}/ack`, {}), sessionId) }
  private parseStatus(value: unknown, sessionId: string): SearchStatus {
    if (!record(value) || value.session !== sessionId || !text(value.intent_id) || !text(value.state) || !Array.isArray(value.tasks) || !value.tasks.every(task) || !Array.isArray(value.candidates) || !value.candidates.every(finding) || (value.detection_workers !== undefined && (!Array.isArray(value.detection_workers) || !value.detection_workers.every(worker)))) throw new Error('The relay returned an invalid search status.')
