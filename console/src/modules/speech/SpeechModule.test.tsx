...
import type { TranscriptClient, TranscriptRequest, TypedUtteranceRequest, VoiceOutcome } from '../../voice/client'
...
class QueuedTranscriptClient implements TranscriptClient {
  ...
  readonly typedRequests: TypedUtteranceRequest[] = []
  ...
  async transcribe(request: TranscriptRequest): Promise<VoiceOutcome> {
    ...
    return this.respond(request.sessionId, request.correlationId)
  }

  async compileText(request: TypedUtteranceRequest): Promise<VoiceOutcome> {
    this.typedRequests.push(request)
    return this.respond(request.sessionId, request.correlationId, 'typed')
  }

  private respond(sessionId: string, correlationId: string, source?: 'typed'): VoiceOutcome {
    const next = this.queue.shift()
    if (!next) throw new Error('No transcript was queued.')
    if (next instanceof Error) throw next
    const plan = next.plan ? { ...next.plan, session: sessionId, correlation_id: correlationId } : next.plan
    return { ...next, source: source ?? next.source, session: sessionId, correlation_id: correlationId, plan }
  }
}
...
function mount(options: {
  ...
  const element = () => (
    <App
      ...
      services={{ transcript: options.transcript, navigation: options.navigation, multiview: options.multiview, voice }}
    />
  )
...
test('typed text uses the relay semantic compiler and never local matching', async () => {
  ...
  relay.answer(outcome({ source: 'typed', transcript: 'Go to the atrium.', plan: relayPlan({ transcript: 'Go to the atrium.' }) }))
  mount({ transcript: relay })

  await compileTyped(user(), 'Go to the atrium.')

  await waitFor(() => expect(planCard()).toHaveTextContent('Go to the atrium.'))
  expect(relay.typedRequests).toHaveLength(1)
...
test('a semantic review with a stale catalog cannot mint a preview', async () => {
  const relay = new QueuedTranscriptClient()
  const review = relayPlan({
    kind: 'review',
    transcript: 'Go to the atrium.',
    steps: [],
    expires_at_ms: null,
    plan_digest: null,
    review: { kind: 'navigate', catalog_identity: 'accepted-catalog-7', destination_id: 'atrium-front' },
  })
  relay.answer(outcome({ source: 'typed', transcript: review.transcript, plan: review }))
  mount({ transcript: relay })

  await compileTyped(user(), review.transcript)

  await waitFor(() => expect(planCard()).toHaveTextContent('accepted destination catalog changed'))
  expect(screen.getByRole('button', { name: 'Prepare current review' })).toBeDisabled()
  expect(screen.queryByRole('button', { name: 'Draft for confirmation' })).not.toBeInTheDocument()
})

test('a semantic multiview review starts the shared photo-route review without reselecting stops', async () => {
  const relay = new QueuedTranscriptClient()
  const pin = { version: 'v1', contentSha256: 'a'.repeat(64) }
  const catalog: NavigationCatalog = {
    session,
    catalogVersion: 'accepted-catalog-7',
    receivedAt: T0,
    expiresAt: T0 + 60_000,
    map: { mapId: 'map', floorId: 'floor', frame: 'world', accepted: true, approvalId: 'approval', mapPin: pin, geometryPin: pin, navigationPin: pin },
    configVersion: 'motion-v1',
    motionConfig: { fixture: true },
    destinations: ['atrium-front', 'carpet'].map((zoneId) => ({ zoneId, name: zoneId, aliases: [], floorId: 'floor', excluded: false, reachability: 'reachable', allowedClasses: ['aircraft'] })),
  }
  const navigationSnapshot: ReturnType<NavigationClient['getSnapshot']> = {
    status: 'ready', reason: null, catalog, preview: null, reviewSupported: true,
  }
  const navigation: NavigationClient = {
    getSnapshot: () => navigationSnapshot,
    subscribe: () => () => {},
    requestPreview: async () => { throw new Error('not used for a multiview review') },
  }
  const preview = vi.fn(async (request: MultiviewPreviewRequest): Promise<MultiviewPreview> => ({
    previewId: 'photo-review',
    intentId: request.intentId,
    previewHash: 'b'.repeat(64),
    expiresAt: T0 + 15_000,
    execution: { planHash: 'c'.repeat(64), mapPin: pin, geometryPin: pin, navigationPin: pin, approvalId: 'approval', configurationSha256: 'd'.repeat(64), permissionZoneIds: ['atrium-front', 'carpet'] },
    views: request.viewpoints.map((view, index) => {
      const point = { xM: index, yM: 0, zM: 1, floorId: 'floor', frame: 'world' as const }
      return { ...view, route: { target: request.selected[0], waypoints: [point], arrivalSlot: { slotId: view.zoneId, zoneId: view.zoneId, position: point }, holdBehavior: 'hover' }, capture: { roomId: view.zoneId, pattern: 'single_still' } }
    }),
  }))
  const multiview: MultiviewClient = {
    preview,
    confirm: async (value) => value.previewId,
    status: async (value) => ({ workflowId: value.previewId, intentId: value.intentId, status: 'pending_confirmation', views: [] }),
  }
  const review = relayPlan({
    kind: 'review',
    transcript: 'Photograph the atrium front and carpet.',
    steps: [],
    expires_at_ms: null,
    plan_digest: null,
    review: { kind: 'multiview', catalog_identity: catalog.catalogVersion, destination_ids: ['atrium-front', 'carpet'] },
  })
  relay.answer(outcome({ source: 'typed', transcript: review.transcript, plan: review }))
  mount({ transcript: relay, navigation, multiview })

  await compileTyped(user(), review.transcript)

  await waitFor(() => expect(preview).toHaveBeenCalledTimes(1))
  expect(preview.mock.calls[0]?.[0].viewpoints.map((view) => view.zoneId)).toEqual(['atrium-front', 'carpet'])
  expect(await screen.findByRole('button', { name: 'Confirm photo route' })).toBeEnabled()
})

test('typed text without a relay compiler reports an unavailable route and emits nothing', async () => {
  mount()

  await compileTyped(user(), 'Go to the atrium.')

  await waitFor(() => expect(screen.getByRole('alert')).toHaveTextContent('The relay has no transcription endpoint on this console. Type the utterance instead; nothing is emitted from an empty transcript.'))
