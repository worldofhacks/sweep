...
describe('Control › Commands, Fleet and the mission tracker', () => {
  ...
  test('the Fleet pane shows registry rows with metrics and reasons, the departed list, and a rejoin', async () => {
    ...
    expect(registry.getAllByRole('article')).toHaveLength(4)
    expect(registry.getByRole('button', { name: 'Deselect D-01' })).toHaveTextContent('Selected')
    expect(clients.console.sent).toHaveLength(0)
    ...
    expect(d03.getByRole('button', { name: 'Select D-03' })).toHaveTextContent('Unavailable')
    ...
    expect(screen.getByRole('region', { name: 'Fleet departures' })).toContainElement(departed)
