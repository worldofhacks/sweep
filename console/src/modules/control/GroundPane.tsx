...
export function GroundPane({ controller }: Pick<ModuleProps, 'controller'>) {
  ...
  return <section className="ct-work-card ct-ground" aria-label="Ground controls">
    <h3>Ground robot</h3>
    ...
    <div className="ct-ground-actions">{(['forward', 'left', 'right'] as GroundDirection[]).map((direction) => <button type="button" className="ct-control-btn" key={direction} disabled={reason !== null}
    ...
    <button type="button" className="ct-control-btn" disabled={returnReason !== null} onClick={() => controller.prepareIntent({ name: 'come_home', args: {} })}>Preview configured ground return</button>
    ...
    <button type="button" className="ct-control-btn" onClick={() => controller.issueHold()}>HOLD selected devices</button>
  </section>
}
