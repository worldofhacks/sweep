...
function OpenCaptures({ captures, label }: { captures: OpenCapture[]; label: DeviceLabeller }) {
  return (
    <section aria-label="Captures in progress">
      ...
      {captures.map((capture) => (
        <p key={`${capture.drone_id}-${capture.connection_epoch}-${capture.capture_id}`} className="cap-open mono">
          {capture.capture_id} · {label(capture.drone_id)} · {filesLabel(capture.files)} captured,{' '}
