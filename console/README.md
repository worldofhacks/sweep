...
in the dock until the operator confirms the exact envelope; the rest send at once. Names outside
the M2.0 set stay pressable so the relay's `unsupported` refusal is recorded rather than hidden.
