...
def test_custom_readings_are_rate_limited_but_hardware_authority_loss_is_signed_now():
    node, clock = make_node()
    join(node)
    while not node._outbound.empty():
        node._outbound.get_nowait()
    original_status = node.device.status
    node.device.status = lambda: replace(original_status(), extras={"scan_age_ms": 10})
    clock.seconds += 0.05
    node._emit_node_status_if_changed()
    assert node._outbound.empty()
    clock.seconds += 0.16
    node._emit_node_status_if_changed()
    frame = node._outbound.get_nowait()
    assert frame["device_telemetry"]["scan_age_ms"] == 10
    assert node._outbound.empty()

    # An interlock revokes authority before the next custom-telemetry period.
    node.device.disable()
    clock.seconds += 0.01
    node._emit_node_status_if_changed()
    readiness = node._outbound.get_nowait()
    assert readiness["type"] == "membership" and readiness["action"] == "readiness"
    assert readiness["control_authority"] is False
    signature = readiness.pop("signature")
    assert protocol.sign_event(readiness, "test-key") == signature
    assert node._outbound.get_nowait()["control_authority"] is False
    assert node._outbound.empty()
