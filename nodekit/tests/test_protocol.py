...
@pytest.mark.parametrize("speed", [-250, 250])
@pytest.mark.parametrize("duration", [100, 500])
def test_body_pulse_command_accepts_signed_bounds(speed: int, duration: int) -> None:
    frame = command(operation="body_pulse", args={"forward_mm_s": speed, "duration_ms": duration})
    assert frame.verifies(KEY)
    assert admit(frame).admitted
    assert "body_pulse" in protocol.MOTION_OPERATIONS


@pytest.mark.parametrize(
    ("speed", "duration"),
    [(0, 100), (251, 100), (-251, 100), (250, 99), (250, 501), (True, 100), (1, True)],
)
def test_body_pulse_rejects_unbounded_or_boolean_arguments(speed: object, duration: object) -> None:
    with pytest.raises(protocol.ProtocolError):
        command(operation="body_pulse", args={"forward_mm_s": speed, "duration_ms": duration})
