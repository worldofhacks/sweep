...
The reference node: everything a vehicle needs to become a device on a relay session, so each new device class implements the existing protocol with explicit capabilities. `adapters/dji_mini3/fake_node.py` is this kit plus a Mini 3's operations; `adapters/ohmni/` packages this kit with the measured Android robot runtime.
...
`capabilities` is the device's own claims. A ground vehicle must claim `ground_drive` and an aircraft `flight` before the relay calls it ready; `lidar`, `camera`, `neck`, `speech`, `lights`, and `screen` describe supported interfaces. Each requires matching runtime evidence and admission; the Ohmni implementation always requires working LiDAR for wheel motion. The kit appends exactly one `class:<device_class>` entry, which is how the join declares its class without any frame gaining a key (`relay/README.md`).
...
- **Execute** `goto` through `move_to`, `rotate_to` through `rotate_to`, `hover` through `stop()`, and `estop` through `stop()` then `disable()`. A ground implementation with `robot_peripheral_v1` and its explicit per-kind capabilities may also execute bounded neck/lights/speech/screen requests. Every other operation fails with `unsupported_operation` naming the class, unless the node overrides `supported_operations`. A motion that outlives the command TTL stops the device and fails with `motion_timeout`; a motion the device abandons fails with `motion_failed`.
...
The optional `node_status.device_telemetry` payload carries bounded diagnostic JSON (16 KiB, depth four), including actual hardware evidence and explicit unknown values. It never replaces signed readiness. `robot_peripheral` preserves the wheel-motion generation; stationary speech/lights/screen do not enable wheels, and all peripheral requests require a nominal watchdog lease. Vendor submission is distinguished from measured physical completion.
...
uv run python -m nodekit.cli --device-id 11 --device mypackage.device:build \
  --relay ws://configured-relay:8010 --session actual-session
```

`--device` is an explicit real `module:factory` path. Relay, session and the device's
own key (`SWEEP_NODE_KEY` or `--key-file`) are required; there is no default fake node,
demo session or fallback to a console credential. Home and local-operator attestations
default false. Fixture classes carry `test:synthetic`, which production relays reject,
and are excluded from the Ohmni payload. Use them only in isolated tests.

The production Ohmni entry point is `python -m adapters.ohmni`, which also owns the local
STOP/spotter page and validates the actual private device configuration. Its camera streams
report their configured identities and advancing publisher evidence in custom telemetry;
the legacy aircraft-shaped camera-capabilities frame is omitted when calibrated gimbal/FOV
