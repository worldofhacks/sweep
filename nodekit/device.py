...
methods, so a new device class costs one file rather than a protocol change.
...
class DeviceStatus:
    ...
    # Device facts that have no telemetry field yet (battery_voltage, docked, ...). The
    # kit never puts them on the wire, so adding one is not a contract change.
