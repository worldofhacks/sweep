...
class FakeAircraft:
    ...
    def __init__(
        ...
        *,
        ...
        self.home = home
        self.capabilities = list(dict.fromkeys((*capabilities, "test:synthetic")))
    ...
    def body_pulse(self, forward_mm_s: int, duration_ms: int) -> None:
        """Model displacement only; hardware owns pulse timing and neutralization."""
        if "body_pulse_v1" not in self.capabilities:
            raise ValueError("body_pulse_v1 is unavailable")
        if self.state != "hovering":
            raise ValueError("body pulse requires a hovering aircraft")
        if (
            type(forward_mm_s) is not int
            or type(duration_ms) is not int
            or not 0 < abs(forward_mm_s) <= 250
            or not 100 <= duration_ms <= 500
        ):
            raise ValueError("body pulse requires bounded integer speed and duration")
        distance = forward_mm_s * duration_ms / 1_000_000
        heading = math.radians(self.yaw_deg)
        self.x += math.sin(heading) * distance
        self.y += math.cos(heading) * distance
