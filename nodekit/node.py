...
class NodeConfig:
    ...
    def __post_init__(self) -> None:
        if (
            type(self.home_pose_confirmed) is not bool
            or type(self.safety_operator_present) is not bool
        ):
            raise ValueError("home and local-operator attestations must be explicit booleans")
        if self.device_id <= 0:
...
class Node:
    ...
    def __init__(self, config: NodeConfig, device: Device) -> None:
        ...
        self._last_node_status_at = float("-inf")
    ...
    def supported_operations(self) -> frozenset[str]:
        ...
        peripheral = getattr(self.device, "run_peripheral", None)
        kinds = getattr(self.device, "supported_peripherals", None)
        if (
            self.device.device_class == "ground_vehicle"
            and callable(peripheral)
            and callable(kinds)
            and kinds()
            and "robot_peripheral_v1" in self.device.capabilities
        ):
            return KIT_OPERATIONS | {"robot_peripheral"}
    ...
    def start_operation(self, command: protocol.Command) -> Execution:
        ...
        if operation == "robot_peripheral":
            ...
            args = peripheral_arguments(command.args)
            kinds = getattr(self.device, "supported_peripherals", lambda: ())()
            if (
                self.device.device_class != "ground_vehicle"
                or args["kind"] not in kinds
                or "robot_peripheral_v1" not in self.device.capabilities
            ):
                return self._unsupported(operation)
            return Execution(status=protocol.COMPLETED, detail=self.device.run_peripheral(args))
        if operation == "goto":
    ...
    def node_status_body(self) -> dict[str, Any]:
        ...
        return {
            ...
            "authority_change_reason": (
                self._authority_change_reason or status.extras.get("authority_change_reason")
            ),
    ...
    def _handle_membership(self, frame: dict[str, Any]) -> None:
        ...
        if self._failsafe_latched:
            ...
            self._watchdog.arm()
            if self.config.home_pose_confirmed and self._safety_operator_present:
                control_authority = bool(self.device.enable())
            else:
                # Joining for real diagnostics must not enable a device before
                # its independent local operator and launch evidence exist.
        ...
        if getattr(self.device, "publish_camera_capabilities", True):
            self.emit(self.capabilities_frame())
        body = self.node_status_body()
        self._last_node_status = body
        self._last_node_status_at = self.config.monotonic()
    ...
    def _emit_node_status_if_changed(self) -> None:
        ...
        if body == self._last_node_status:
            return
        previous = self._last_node_status
        now = self.config.monotonic()
        # Raw sensor ages/ranges may change on every watchdog tick. Publish those at
        # most 5 Hz, while authority, watchdog and publisher transitions remain immediate.
        if previous is not None:
            core = {key: value for key, value in body.items() if key != "device_telemetry"}
            old_core = {key: value for key, value in previous.items() if key != "device_telemetry"}
            if core == old_core and now - self._last_node_status_at < 0.2:
                return
            if body["control_authority"] != previous["control_authority"]:
                # Advisory readings do not mutate relay readiness. Sign the hardware's
                # authority transition so a local interlock also stops remote planning.
                self.emit(self._readiness_frame(self._connection_epoch, body["control_authority"]))
        self._last_node_status = body
        self._last_node_status_at = now
        self.emit(self.node_status_frame(body))
    ...
    def _handle_command(self, raw: dict[str, Any]) -> None:
        ...
        if command.operation != "robot_peripheral":
            self._motion_generation += 1
    ...
    async def _run_command(self, command: protocol.Command, generation: int | None = None) -> None:
        ...
        pulse_stale = command.operation == "body_pulse" and (
            command.seq != self._last_seq or command.connection_epoch != self._connection_epoch
        )
        if generation != self._motion_generation or pulse_stale:
            self.emit(
                self._acknowledgement(
                    command,
                    protocol.INVALIDATED if command.operation == "body_pulse" else protocol.FAILED,
                    protocol.STALE_COMMAND
                    if command.operation == "body_pulse"
