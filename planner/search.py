...
from planner.navigation_contracts import free_grid_covers_pose, volume_contains
...
class SearchArea:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.floor_z_m, bool)
            or not isinstance(self.floor_z_m, int | float)
            or not math.isfinite(self.floor_z_m)
        ):
            raise ValueError("search floor elevation must be finite")
        object.__setattr__(self, "floor_z_m", float(self.floor_z_m))
...
class SearchPlanner:
    ...
    def plan(
        ...
        if (
            request.motion.swept_radius_m > artifact.grid_clearance_m
            or request.motion.swept_half_height_m > artifact.grid_clearance_m
        ):
            return SearchRefusal(
                "clearance_exceeds_geometry",
                "search aircraft envelope exceeds known-map clearance",
            )
        level = self._level_for(
            request.zone, artifact.grids, request.zone.floor_z_m + request.camera.height_agl_m
        )
    ...
    @staticmethod
    def _level_for(
        area: SearchArea, grids: tuple[GridLevel, ...], camera_z_m: float
    ) -> GridLevel | None:
        candidates = tuple(grid for grid in grids if grid.floor_id == area.floor_id)
        return (
            min(candidates, key=lambda grid: (abs(grid.z_m - camera_z_m), grid.z_m))
            if candidates
            else None
        )
