...
def load_navigation_deployment(
    ...
    if backend not in {"synthetic", "remote"}:
        raise SettingsError("navigation backend must be synthetic or remote")
    ...
    deployment.runtime.maximum_aircraft = max_aircraft
    deployment.runtime.require_phone_authorization = backend == "remote"
...
def _arrival_slots(
    ...
    for raw in value:
        ...
        if item["navigation_allowed"]:
            navigation_allowed_zone_ids.add(zone_id)
