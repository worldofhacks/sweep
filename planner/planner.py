...
def _formation_targets(
    ...
    *,
    ...
    """
    if name not in FORMATION_NAMES or not 2 <= len(selected) <= 6:
        return None
    ...
    return _sequential_formation_order(
        tuple(assignments),
        snapshot,
...
def _class_formation_targets(
    ...
    *,
    ...
    if (
        ...
        or not 1 <= count <= 6
        or (name in {"wedge", "diamond"} and 1 < count < 4)
    ):
        return None
    ...
    if not all(isfinite(value) for value in (center_x, center_y, z, spacing)):
        return None
    offsets = ((0.0, 0.0),) if count == 1 else _formation_offsets(name, count)
    if offsets is None:
        return None
...
def _sequential_formation_order(
    ...
    *,
    ...
    occupied = {
        ...
        for drone_id, aircraft in snapshot.aircraft.items()
        if aircraft.membership is MembershipState.READY and aircraft.mobile
    }
