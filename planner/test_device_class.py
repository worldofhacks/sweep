...
from planner.planner import (
    ...
    FORMATION_SPACING_CLEARANCE_FACTOR,
...
def test_a_formation_places_each_class_around_its_own_centre() -> None:
    ...
    assert abs(targets[11][0] - targets[12][0]) == pytest.approx(
        snapshot.spacing * FORMATION_SPACING_CLEARANCE_FACTOR
    )
    assert abs(targets[1][0] - targets[2][0]) == pytest.approx(
        snapshot.spacing * FORMATION_SPACING_CLEARANCE_FACTOR
    )
