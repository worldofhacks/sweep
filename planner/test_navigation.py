...
def test_runtime_dispatch_acceptance_is_separate_from_preview_evidence() -> None:
    ...
    live = NavigationLiveState(
        plan.roster_version,
        plan.plan_revision,
        tuple(drone.drone_id for drone in plan.selected),
        plan.selected,
        plan.config,
        plan.permission,
    )
    assert (
        planner.revalidate(plan, map_artifact, live, 0, 0, 0.05).code == "artifact_not_dispatchable"
    )
    ...
    assert planner.revalidate(plan, map_artifact, live, 0, 0, 0.05, acceptance=acceptance) is None


def test_runtime_dispatch_acceptance_rejects_changed_navigation_pin() -> None:
    from planner.navigation import ArtifactPin, NavigationDispatchAcceptance
    ...
    acceptance = NavigationDispatchAcceptance(
        ...
        plan.map_pin,
        plan.geometry_pin,
        ArtifactPin("other", "a" * 64),
        plan.plan_revision,
    )
    refusal = planner.revalidate(plan, map_artifact, live, 0, 0, 0.05, acceptance=acceptance)
