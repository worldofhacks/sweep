...
The owner approved this tag-map baseline on 8 September after checking physical measurements. The checkpoint retains the original map bytes and a separate [approval record](../deployments/real-navigation/map-approval.json) tied to their SHA-256. Wall and height measurements and verification of replacement tags remain outstanding. Camera and mount calibration still need hardware validation; map approval alone does not establish route clearance.
...
Retain the approved tag-map baseline while completing the wall, height and replacement-tag measurements. Use those measurements to generate the static flight grid, routes and geofence through the existing bundle tooling. Validate camera intrinsics, the fixed-head mount and world-frame alignment on hardware. Exercise LiDAR guards and stop controls during the map test. The [hardware checkpoint guide](hardware-navigation-checkpoint.md) separates these remaining measurements from software verification.
