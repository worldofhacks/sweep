...
Scope: Mini 3 provides only downward vision sensing, and DJI does not document Virtual Stick obstacle avoidance for this aircraft. Use wall-mounted tag36h11 AprilTags as the shared indoor localization source, detected through `apriltag-python` or `cv2.aruco`. Its controllable gimbal range is -90° to +60°, which does not point far enough upward to reliably view ceiling-mounted tags, so the placement design is wall-only. Reacquire tags periodically and fuse each fix with VPS/IMU dead reckoning between detections. Calibrate each camera and imaging configuration used for localization evidence. Before any autonomous room traversal, integrate the pose estimate and independent forward, rear, lateral, upward, and downward clearance observations into the arbiter. Compare localization with surveyed points and stage obstacles at the minimum-clearance boundary and within the configured stopping distance. A stale, missing, or contradictory source fails closed. Tag placement uses a validated M2.6 occupancy map and room graph or a validated supplied floor plan; Marble output never supplies placement geometry.
Done when: calibration artifacts for every camera and imaging configuration, tag detection, and periodic re-acquisition run against the surveyed layout; across five complete mapped-route rehearsals, every aircraft remains localized with p95 position error at or below 0.25 m and no unhandled update gap over 500 ms. In 20 staged approaches per protected direction, every obstacle inside the stopping envelope is detected before command dispatch, no test produces a false-clear result, and injected stale or missing data commands hold. Until both gates pass, `map_area` returns `unsupported`. [DJI Virtual Stick obstacle-avoidance support](https://developer.dji.com/api-reference-v5/Components/IVirtualStickManager/IVirtualStickManager.html) · [Mini 3 sensing specifications](https://www.dji.com/mini-3/specs)

Owner decision: add floor metadata to the validated room graph, associate each tag with its graph floor, and keep one flat building-wide tag-ID namespace. Do not reserve fixed ID ranges per floor; dense floors may need 50 or more tags and exhaust an assigned range.
