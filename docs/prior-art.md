...
4. **The AprilTag localization decision (PR #66) survives scrutiny**, with two mandatory engineering additions: **EKF with delayed-measurement replay** fused with MSDK velocity (uncompensated video latency alone eats 12–28 cm of the 25 cm budget at 0.5 m/s) and **multi-tag joint PnP** with ambiguity gating. Detection compute is a non-issue (measured 12–33 ms/frame at 2.7K; the real 720p feed costs ~4× less). **Correction (2026-09-04): the localization input is the Mini 3 live downlink, which is 720p/30 (DJI O2 Live View Quality, [specs](https://www.dji.com/mini-3/specs)) — not the 2.7K/4K recording resolution this section's range math first used. At 720p, per-tag detection range roughly halves; see the Localization section for corrected numbers.** Motion blur is the top physical risk. Every rival modality fails on cost, accuracy, or stock-hardware grounds; SLAM/VIO is REJECT for MVP (no synced IMU, no metric scale).
...
> **Correction (2026-09-04) — plan for a 1280×720 @ 30 fps feed, not 2.7K.** The Mini 3's off-board video is the DJI O2 transmission stream: Live View Quality **720p/30fps** ([DJI Mini 3 specs](https://www.dji.com/mini-3/specs)); the 2.7K/4K modes are SD-card recording only and are not available live. WildBridge's WebRTC capturer "targets 1080p" but only by scaling the same 720p decoded live view — no added detail. This doc's first version ran the range math at 2.7K; corrected planning basis: focal ≈ **843 px** at 1280×720 (82.1° diagonal FOV) vs ~1770 px at 2.7K, so every pixel-count-derived range below shrinks by ~2.1×. Consequences: (a) detection compute gets ~4× cheaper (run `quad_decimate=1` — at 720p the decimated quad search would run at 640×360 and cost small tags, and full-res is now trivially affordable); (b) **detection range roughly halves**: a 16 cm 36h11 tag is findable to ~3.4 m and pose-grade (≥60 px) to only ~2.2 m; 20 cm → ~4.2 m find / ~2.8 m pose-grade; ~30 cm restores the original ~6–7 m envelope; (c) corner-noise-driven pose error at a given distance roughly doubles, making **multi-tag joint PnP and close-range fixes more important, not less**; (d) **floor tags gain**: at the planned 1.5–2.4 m flight band a nadir 16 cm floor tag spans ~47–90 px — comfortably pose-grade — while wall tags at route distances need upsizing to ~24–30 cm or ~2–2.5 m spacing; (e) latency and blur conclusions are unchanged (the measured ~200 ms O2 link and WHIP numbers were always on the real 720p stream; blur smear and module size scale together). Calibrate intrinsics at exactly 1280×720 through the exact delivery pipeline.
...
- **Pose quality**: 36h11 at 16–20 cm is detectable to ~3.4–4.2 m at the real 720p feed (~7 m at 2.7K was the uncorrected figure; documented formula); published error data shows ~1–10 cm per fix in range, with **camera-yaw obliqueness the dominant error source** and single-tag IPPE ambiguity flips a real failure mode ([apriltag#71](https://github.com/AprilRobotics/apriltag/issues/71)). Mitigations to adopt: **multi-tag joint PnP against the surveyed map** (the [PhotonVision MultiTag](https://docs.photonvision.org/en/latest/docs/apriltag-pipelines/multitag.html) pattern) whenever ≥2 tags are visible; for single tags, err1/err2-ratio + surveyed-normal prior + Mahalanobis gating against the dead-reckoned prior; drop fixes below ~40 px or low `decision_margin`. Placement planner (#58) target at 720p: **≥1 tag within ~2–2.5 m of every route point for 16–20 cm tags, or upsize route/wall tags to ~24–30 cm to restore 3.5–4 m spacing** (two in view where cheap), mixed sizes near precision zones; nadir floor tags in formation zones are unaffected (16 cm is pose-grade across the whole 1.5–2.4 m altitude band).
...
- **Wall vs floor tags — needs one reconciliation decision.** PR #66 chose wall-mounted (rationale: gimbal −90°…+60° can't see ceiling tags — correct). The newer 416 Congress system plan circulating in the team ("indoor_swarm_system_v2") uses **floor grids + wall tags at −50° gimbal**, which the gimbal also supports and which the accuracy literature slightly favors (near-nadir floor tags at short range beat oblique wall tags at distance). **The 720p correction strengthens the floor-tag side**: nadir floor tags sit at fixed 1.5–2.4 m range (pose-grade at 16 cm even at 720p), while wall tags lose half their usable range and need upsizing or densifying. Everything in this section (EKF, MultiTag PnP, blur, calibration, density targets) applies identically to both; the tag-map format should carry per-tag mounting normal either way. Flagged in Open Questions.
...
| #58 tag placement | Visibility heuristic from validated geometry | Range formula + published error data | **Add targets (720p-corrected)**: ≥1 tag in ~2–2.5 m everywhere for 16–20 cm tags (or 24–30 cm tags at 3.5–4 m spacing), 2 in view where cheap, mixed sizes at precision zones, ≤45° incidence; resolve floor-vs-wall first |
...
5. **E5 — Tag localization dry run** (feeds #57): intrinsics through the live 1280×720 pipeline (RMS <0.5 px) → latency distribution (p95 <500 ms, jitter <150 ms) → static accuracy at 8–10 surveyed points, 1–4 m (the 720p pose-grade envelope for 16–20 cm tags), 0–45° incidence (go/no-go p95 <0.15 m within 2.5 m; flip-rejection working) → dynamic taped-route run at 0.5 m/s with the full EKF (p95 ≤0.25 m, zero unhandled gaps — a dress rehearsal of the actual M3.0 gate) → blur curve (detection rate vs speed; <80% at 0.5 m/s ⇒ lighting/shutter/speed response). Optionally cross-checked by the $50 IR-webcam rig as independent ground truth.
