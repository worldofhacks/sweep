...
The phone contracts preserve diagnostic status for raw Android data and enforce execution admission. The revised recording helper required an ownership fix, detailed below. The camera path has a real probe implementation behind the camera port, and the route path reaches the flight executor only after signed admission and setup matching.
...
Different working roots could acquire independent locks while controlling the same MediaMTX container. The author replaced that path with project/container identity locks in `01963c8a696023255748b9eeca01ea0c9a279219`, which also checks configuration stability during startup. Review preserved those changes. Follow-up `979e5a44ecfe3941ec286c790cfce491d9d7272e` refuses an already-running service before creating a run or entering cleanup, and isolates the lock regression. All 18 recording tests passed with real Docker, FFmpeg, and ffprobe.
