...
The backend holds the World API key. The completed human-phone fallback enters this path directly. Drone acquisition first executes a confirmed `capture_room` or `map_area` mission through the normal planner and arbiter path, then attaches the resulting files and poses to the capture record. A room world can be viewed or composed, but it cannot emit an intent or provide geometry to the planner or arbiter.
...
| Swarm adapters | Python | Autonomy | `sim` in M1 implements the flight and camera contracts. Additional ROS 2 or MAVLink vehicles are Future work after a concrete second hardware need. |
...
- **Room-world lane:** the manual three-photo flow is completed precursor evidence and a fallback. M1 proves one spoken drone capture and private room generation. M2 adds a persistent 3-to-5-room project and scales the proven bridge. M3 proves known-map autonomous multi-room traversal and capture with one drone and then two. M4 finishes one operator-composed, reviewed walkthrough and stores its MP4.
