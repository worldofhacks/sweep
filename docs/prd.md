...
3. Button control of four physical DJI Mini 3 drones with live session membership, with 4 to 6 drones proven in simulation through the same Intent v1 contract. Spoken language and gestures follow as additional producers.
...
| Video glass-to-glass latency (laptop) | measured < 300 ms WebRTC; measure and report HLS fallback latency, with its threshold open; report aircraft-to-controller, Android processing, and LAN segments |
...
| Plan compiler (LLM) | Python | Platform | Turns language into an ordered list of intents using structured output; never touches commands. Work may begin after M1.E, with acceptance in M4. |
...
| Media server | MediaMTX | Platform | Ingest drone video, serve WebRTC with an HLS fallback, and record. M3. |
...
- Scheduling: language work may begin after M1.E. Hardware safety work takes priority until M2.0 passes. After M2.0, gesture, indoor autonomy, the third and fourth physical nodes, and the 4-to-6-drone simulator expansion may proceed concurrently; the input lane may land first.
...
- M2.0 safety and evidence: keep the complete arbiter, network stop, state and confirmation checks, live registry, roster and connection epochs, geofence, ceiling, spacing, battery, link-loss and positioning-loss behavior, append-only JSONL audit log, and the section 8.5 flight rule. The formation library, altitude gesture, sweep planner, detector, mosaic, language and LLM work, replay UI, metrics dashboard, session report, and release polish remain outside the checkpoint.
...
| Video bandwidth fights control links | high | medium | dual-band plan, low-latency HLS fallback, capture-card FPV as final fallback |
