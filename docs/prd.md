...
Version 0.7. Delivery is organized into three capability areas: Interaction, Autonomy, and Platform. Engineers claim ready work per task; capability areas coordinate module boundaries and review.

**Current status.** [Audit #224](https://github.com/worldofhacks/sweep/pull/224) records 2,201 Python tests, 528 console tests, and three browser proofs. This is software evidence. Implementation status follows each linked PR; open changes still require review and merge. This audit does not establish physical flight, localization, camera or hardware acceptance.

**Authority.** GitHub issues set current ownership, dependencies, and delivery status. This PRD records the issue-defined product boundary and retains historical milestone IDs. Current issue decisions override conflicting historical scope, dependencies and status here; software tests demonstrate code behavior, while a physical claim requires the issue's recorded acceptance evidence.

The current implementation priority is autonomous destination navigation and explicit object search: `navigate {zone_id}` and `search {zone_id, target_class}`. The room-world path remains part of the product vision and has a separate team lane; it is deprioritized for the immediate navigation proof. Issue #89 remains the formal stretch milestone for autonomous search detection and does not gate the MVP.
...
An operator selects aircraft, asks them to travel to an approved destination or search an approved area, reviews the exact server-frozen plan and confirms it. Destination navigation ends in hold; object search reports coverage and localized findings for acknowledgement without detection-driven motion. Console buttons, keyboard controls, accepted gestures and voice use the same intent, planner, arbiter and adapter path. The current software demonstration uses four signed simulated nodes. The physical target remains four Mini 3/RC-N1/Android sets, accepted one aircraft before two and four; 4–6 aircraft remain in simulation.

The supplied metric map, surveyed tags, measured sensor/timing inputs and deterministic clearance checks define the allowed flight volume. Flight authorization requires the independent-reference localization and physical failure evidence in #83–#86/#145. Room names and aliases live in accepted map metadata; tag IDs remain building-wide. Navigation destinations and formation volumes carry separate permissions.

Capture/Worlds is a separate team-owned product lane. It preserves the three-photo feasibility result and develops confirmed drone capture, pilot-assisted survey, known-map traversal/capture and room-world presentation. The longer-term “Map this floor” workflow remains under #63/#100. Its media and generation jobs do not gate the immediate navigation/search software proof, and generated geometry never supplies flight safety state.

This MVP prioritizes recorded capability demonstrations and every safety control needed around real aircraft. Production governance and operational hardening remain follow-on work. Mandatory five-route localization acceptance stays in the physical flight gate.
...
**Value.** The operator can send selected aircraft to known indoor destinations, inspect live views, and review search findings through a shared safety boundary. The room-world lane preserves captured room evidence and its generated presentation separately.
...
1. Configured destination navigation with authoritative room/alias resolution, selected airborne aircraft, exact route preview, clearance revalidation and arrival/hold evidence.
2. Coverage-tracked search of approved space with timestamped detections, measured pose/camera provenance, five-frame find localization and operator acknowledgement without unplanned motion.
3. Button control of four physical DJI Mini 3 drones with live session membership, with 4 to 6 drones proven in simulation through the same Intent v1 contract. Webcam gesture work and the transcript-to-plan compiler develop against that contract in parallel; speech development follows #42/#124, with live producer acceptance tracked separately.
...
7. A runnable public repository with the console, relay, planner, adapters, demo fixtures and evals; a separate Capture/Worlds lane preserves capture bundles, private generation jobs and room-world provenance.
...
| Intent to first drone motion (indoor, 1 to 4 physical drones) | < 300 ms; command RTT, jitter, and drops reported separately |
...
| Scripted mission (arm, take off, formation, sweep, come home, land) | completes hands-free in < 3 minutes with 4 to 6 simulated drones and four physical Mini 3 nodes |
| Live fleet membership | 1 running session accepts up to 4 physical nodes; landed and disarmed leave/rejoin preserves history, increments roster and connection epochs, invalidates stale work, and leaves unaffected aircraft stable |
| Demo completion | 1 recorded pass per flight workflow with no safety intervention |

The demo-first acceptance profile requires one recorded pass for each flight workflow and a 20-utterance live language set. Additional repeatability beyond the mandatory M3B five-route localization gate and broad language evaluation move to [F.6 in the delivery plan](mvp-plan.md#f6-harden-the-proof-for-production-use). Every geofence, arbiter, e-stop, separation, clearance, and physical-RC gate remains active.
...
- **Query volume:** one operator; roughly 10 gesture intents per minute during active control, 1 to 2 language commands per minute, Virtual Stick commands at 5 to 25 Hz, measured telemetry rate per aircraft, four physical video streams, and 4 to 6 simulated drones.
...
| Language module | JS and Python | Interaction with Platform | Browser push-to-talk, configured Deepgram/Whisper transcription, authoritative compilation and plan preview use the same intent bus; live producer acceptance remains separate. |
...
| Adapter: `takeoff, goto, land, hover, estop, telemetry` | per drone | acks, telemetry | timeout → hold and alert; link loss → local zero-velocity hold, then configured LAND with physical RC takeover |
...
| `navigate(zone_id)` | one configured destination | a reviewed route to that zone | absent destination configuration, stale state, or an unsafe route → refuse or hold and alert |
| `search(zone_id, target_class)` | one configured destination and an explicit object label | a bounded search request and detection events | absent or ambiguous zone or label, stale state, or unsafe route → refuse or hold and alert |
...
External dependencies: the World Labs World API for room generation, the configured Deepgram or OpenAI Whisper transport and plan-compiler API for language, MediaPipe model download (once), MediaMTX (local), the Android DJI Mobile SDK, four Mini 3 and RC-N1 pairs, four benchmarked Android phones, shared indoor localization, and validated map-plus-pose clearance evidence. `WLT-Api-Key` and `OPENAI_API_KEY` stay in the backend environment. Future input producers and vehicle families may add source-specific SDKs after their access gates pass.
...
| Position evidence permits flight | independent-reference acceptance plus current pinned localization | five-run p95 ≤ 0.25 m; no unhandled gap > 500 ms; runtime age/uncertainty within accepted bounds | refuse further movement, hold and follow configured landing policy |
| The operator is present | current operator/RC authority evidence under the accepted configuration | configured presence deadline | withdraw movement authority and apply the configured hold/landing policy |
...
| Release | Issue | Earned intent set | Status gate |
|---|---:|---|---|
| C1 | #97 | `arm`, `select`, `takeoff`, `translate`, earned `altitude`, `hold`, `come_home`, selected `land`, `land_all`, `estop`, `capture_room` | Profile and deployment evidence remain open. |
| C2 | #98 | C1 plus `disarm`, formation changes, spacing, and `sweep` | Simulation may advertise C2 after its gate; Mini 3 stays C1 until four-node acceptance. |
| C3 | #99 | C2 plus pilot-assisted `survey_area` | The workflow records evidence and grants no autonomous movement. |
| C4 | #100 | C3 plus `map_area` | Requires accepted map, localization, clearance, adapter, and frozen-plan evidence. |

Configured navigation and search have separate deployment gates. Current software adds `navigate {zone_id}` and `search {zone_id, target_class}` only when their deployments are configured. `search` requires both fields from the user and refuses an absent or ambiguous destination or label. `map_area` remains the C4 building-level intent.
...
The authoritative fleet registry is keyed by stable `drone_id` and carries a monotonic `roster_version`, a connection epoch per aircraft, and `registered`, `ready`, `leaving`, `disconnected`, or `degraded` state. Signed join, readiness, graceful-leave, and unexpected-loss events update it. Identity, capabilities, fresh telemetry, home pose, control authority, and RC-safety-operator presence gate readiness. Joining leaves current selection and accepted plan unchanged; the next dispatch applies roster-version validation. Graceful removal requires landed, disarmed, task-free state; it atomically clears selection and invalidates pending confirmations or plans built against the previous roster. Unexpected or airborne loss remains visible and takes the configured hold or fail-safe path. Reconnection increments the connection epoch, and prior-epoch commands or acknowledgements are refused.

### 5.3 Planner

Deterministic and unit-tested: formations (line, column, circle, grid, V) around a center with spacing; translate; altitude; sweep lanes (lawnmower per drone with lane assignment by current position); come home with staggered pads and a second call to land; hold; select; `capture_room`; and `map_area`. Fleet size comes from the live aircraft registry. Planner expansion, arbiter checks, telemetry fan-out, adapter dispatch, and console selection iterate the selected registered aircraft. Every accepted plan records `roster_version`; dispatch refuses a stale roster. Spacing checks cover every ready airborne aircraft, including aircraft outside the command selection. Known-map area capture resolves the room graph and approved capture poses, assigns rooms, plans collision-checked routes, and schedules capture tasks. The demo uses deterministic fixed slots and nearest available aircraft. Everything is clamped to the mode's box before it becomes a command.

### 5.4 Modes

| Mode | Positioning | Box | Spacing | Speed | Notes |
|---|---|---|---|---|---|
| Indoor, constrained | accepted shared localization plus deterministic map-and-pose clearance | defined once per space | 0.8 m | 0.5 m/s until measured evidence supports more | the Mini 3 room-capture mode |
...
The operator console contains fleet controls, route/search previews, live video, findings, input surfaces and connection/safety state. Capture libraries and room-world presentation belong to the separate Capture/Worlds interface. Authoritative flight state comes from the relay. Audit visualization work follows #93 rather than creating a second flight-state owner.
...
The transcript-to-plan compiler uses authoritative relay selection, capabilities, map metadata and motion policy. Push-to-talk recording sends a bounded clip to the relay; the configured transport uses Deepgram Nova-3 or Whisper under #124, with credentials retained on the server. Denied permission, empty audio, upload/provider failures and ambiguity do not emit an intent. The compiler's plan is validated and previewed before confirmation, then emitted through the normal relay/planner/arbiter path. Destination aliases resolve against the accepted catalog; changing selection, destination, map or configuration invalidates the preview. Configured console search already has a software proof; voice search requires its own compiler and producer acceptance. Continuous listening and description-guided queries remain separately tracked extensions.
...
- Scheduling: webcam gesture work proceeds beside M1.3 once the shared input contracts freeze. The transcript-to-plan compiler proceeds against M1.1 relay state and the M1.2 two-drone simulator, while speech capture and provider integration proceed under #42/#124. Hardware safety work takes priority until M2.0 passes. After M2.0, indoor autonomy, the third and fourth physical nodes, and the 4-to-6-drone simulator expansion may proceed concurrently with the active input lanes.
...
- M1 dependency: the issue-defined one-node bridge and capture prerequisites require recorded acceptance before the corresponding M2 hardware gate. Their current status is tracked in the issues.
...
- Physical entry requires accepted M2.0 evidence, including its selected live feed. Software navigation/search development already proceeds against those interfaces. Recording, multi-stream work, detector prototyping, and the active gesture and language lanes run concurrently; relay and console integration wait for their shared contracts. `map_area` remains `unsupported` until shared indoor localization, map-plus-pose clearance and the corresponding C4 evidence pass their gates.
- Deliverables: expand the M2.0 selected-feed proof into four-node MediaMTX ingest, WebRTC/MJPEG serving, recording, detection events, and measured latency (Platform); add the live camera mosaic, focus-by-selection, telemetry and sensor state, detector, attention promotion, and operator confirmation in the console (Interaction); integrate a measured indoor-localization source with camera observations, velocity, height, attitude, gimbal state, timestamps, and update-dropout reporting. The map, route geometry, camera calibration, and timing artifacts are versioned inputs to the acceptance run. Prove localization and deterministic map-plus-pose clearance before exposing `map_area`; then preview and confirm it through the operator console, navigate one drone and then two through approved room poses on a supplied occupancy map, partition known room targets, attach every completed capture bundle to the room catalog, and submit the accepted run to per-room World API jobs with `public: false` (Autonomy with Platform and Interaction support).
- Known-map autonomous flight boundary: the approved Phase 1 area on Level 1 includes the lobby, the declared corridor and kitchen transit, and the atrium-front destination, with open doors, static empty space, no stairs, no people or pets, guarded aircraft, a known launch and return zone, a Sweep operator present, and one physical RC safety operator per active aircraft. The kitchen remains a named room and transit in the map. Formation demonstrations use the lobby and atrium-front boxes only. The complete MVP commits four drones to line, column, wedge and diamond transitions after the accepted route and the #91 BVC gate. Autonomous search detection remains the time-permitting #89 stretch. Capture no longer gates the flight and formation exits. Before `map_area`, the operator imports or creates the occupancy map, validates the room graph and approved capture poses, and approves the geofence. Marble remains downstream of capture.
- Exit: the control panel shows four live cameras, telemetry, sensor events, and membership state; the operator can focus a drone by selection; a detection promotes its feed within one second; every physical source meets the measured latency budget; and the accepted route/formation workflow passes its physical gate. The separate #63/#100 workflow owns traversal-and-capture acceptance. Hardware localization acceptance requires five complete rehearsals of the approved route, each compared with an independently measured reference. The evidence bundle records camera input, velocity, height, attitude, gimbal state, timestamps, update dropouts, fused pose, command stream, JSONL, and video; it identifies the accepted map, geometry, calibration, and timing artifacts. Across those five rehearsals, p95 position error is at or below 0.25 m and no unhandled localization gap exceeds 500 ms. Each route begins only after the live-session, map, geometry, calibration, and safety gates pass. A stale or mismatched artifact, stale localization, or failed live-session gate commands hold and then the configured land or fail-safe path; physical RC pause, takeover, RTH, and landing remain available. Only measured evidence qualifies for the physical gate. The separate #63/#100 traversal-and-capture acceptance additionally requires the following: every reachable room receives one complete pose-anchored capture bundle, no path crosses an occupied cell or minimum-clearance boundary, no separation violation occurs, every aircraft returns or executes its configured fail-safe, no manual flight correction is needed, and the room catalog has no missing, duplicate, or cross-linked captures. For the accepted run, each bundle becomes a successful World API job with `public: false` linked to the same room and its returned room world.

### M4: Language, gesture, and final proof of concept

- Entry: M4.1 compiler work begins against the M1.1 relay-state and M1.2 two-drone sim interfaces. M4.4 gesture development begins against frozen M1.3 input interfaces while M1.3 is underway; M1.3 completion gates integration and acceptance. M4.4 retains its stable historical identifier. M4.3 speech and destination/search software proceed against the accepted interfaces; physical autonomy still requires M2.0 and the measured M3 gates. These lanes may proceed concurrently, with the input lane eligible to land first. Contract and safety gates serialize plan schema, relay state, ordered emission, detection-event shape, shared console integration, and cross-review. M4.5 begins after the accepted input producers and M3 indoor-autonomy exit are complete.
...
- Description-guided search: extend the current explicit `search {zone_id, target_class}` contract with a stored, bounded perception query only after its acceptance evidence exists. Perception emits candidate, progress, and completion events with provenance. Face identity, autonomous following, and autonomous approach remain excluded, and a person validates every candidate.
...
| Positioning or clearance sensing is insufficient indoors | high | severe | Mini 3 downward vision is not obstacle avoidance; keep `map_area` unsupported until shared localization and validated map-plus-pose clearance gates pass |
...
```json
{
  ...
  "name": "arm | disarm | estop | select | takeoff | land | land_all | hold | translate | altitude | formation_next | formation_set | spacing | come_home | sweep | navigate | search | capture_room | survey_area | map_area",
  "args": {},
  "selection": [1, 2, 3],
...
`capture_readiness` is a guidance event. It reports pose, clearance, camera, storage, and missing-coverage readiness. In `visual_advisory` mode, its optional suggestion is limited to yaw or gimbal. XYZ suggestions require accepted `registered_metric` localization and directional clearance. The configured `search` path emits candidate and progress events with provenance.
