...
- Exit: the control panel shows four live cameras, telemetry, sensor events, and membership state; the operator can focus a drone by selection; a detection promotes its feed within one second; every physical source meets the measured latency budget; and the accepted route/formation workflow passes its physical gate. The separate #63/#100 workflow owns traversal-and-capture acceptance. Hardware localization acceptance requires five complete rehearsals of the approved route, each compared with an independently measured reference. The evidence bundle records camera input, velocity, height, attitude, gimbal state, timestamps, update dropouts, fused pose, command stream, JSONL, and video; it identifies the accepted map, geometry, calibration, and timing artifacts. Across those five rehearsals, p95 position error is at or below 0.25 m and no unhandled localization gap exceeds 500 ms. Each route begins only after the live-session, map, geometry, calibration, and safety gates pass. A stale or mismatched artifact, stale localization, or failed live-session gate commands hold and then the configured land or fail-safe path; physical RC pause, takeover, RTH, and landing remain available. Only measured evidence qualifies for the physical gate. Every reachable room receives one complete pose-anchored capture bundle, no path crosses an occupied cell or minimum-clearance boundary, no separation violation occurs, every aircraft returns or executes its configured fail-safe, no manual flight correction is needed, and the room catalog has no missing, duplicate, or cross-linked captures. For the accepted run, each bundle becomes a successful World API job with `public: false` linked to the same room and its returned room world.
