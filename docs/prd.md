...
- Work order: first connect the webcam producer to the relay, planner, arbiter, and sim (Sept 2 to 4). Spoken natural language remains the next control path and runs Sept 5 to 9. After the webcam-to-sim gate, independent M3 media work and M4 corpus or eval work may start concurrently under the provisional schedule below.
...
- Entry: M1's webcam-to-sim safety gate is green and one camera source is available. Media setup and detector prototyping may run beside M1 language work; relay and console integration wait for their shared contracts.
- Deliverables: MediaMTX ingest, WebRTC/MJPEG serving, recording, detection events, and latency measurement (Platform); live camera mosaic, focus-by-selection, telemetry and sensor state, detector, attention promotion, and operator confirmation in the console (Interaction).
...
- Entry: M1's webcam-to-sim safety gate is green. Corpus authoring, cached eval work, and speech fixtures may start beside M1 and M3; resolver and emission integration wait for the M1 plan and relay contracts. M2 has reached the adapter stage when drones are available.
...
- Scheduling decision under review: Koby has directed M3 video and the full M4 language scope to run concurrently. The current estimate is 18 to 23 person-days against 15 gross team-days from Sept 5 through Sept 12, a capacity gap of 3 to 8 person-days pending team confirmation. Contract and safety gates still serialize the plan schema, relay state, ordered emission, detection-event shape, shared console integration, and cross-review. Media setup, detector prototyping, corpus authoring, cached eval fixtures, and speech smoke preparation can proceed in parallel.
...
M1's webcam-to-sim path remains the only product work until its Sept 4 gate is green. Physical inventory may happen when hardware arrives, but it cannot displace the planner, arbiter, sim, relay, schema, console, logging, or CI work.
...
| Sept 5 | Build push-to-talk recording, transcript preview, clarification, confirm, and cancel in the existing console; start independent detector prototyping after the webcam-to-sim gate | Lead one-drone hardware bring-up if the M2 gate is open; otherwise pull forward time-boxed M4 resolver tests that do not gate M1 | Add the relay-side Whisper transcription endpoint and server-held key; define the plan schema; start MediaMTX setup in a separate ready task |
| Sept 6 | Wire Whisper transcripts and confirmed plans to ordered relay emission; add capture, transcription, and UI-path tests; continue isolated M3 console work | Continue hardware bring-up if open; otherwise pull forward M4 resolver success, ambiguity, and refusal cases that do not gate M1 | Integrate transcription and compiler latency and cost logging, `validate_plan`, operator decision logging, ordered emission, cached-response CI, and independent media configuration; defer watchdog work until the compiler-to-sim path is green |

### 8.4 Weeks two and three, by milestone

- **M1 language completion (Sept 7 to 9):** Platform work completes the Whisper API transcription endpoint, transcript-to-plan compiler path, latency and cost logging, cached eval, error handling, and provisional 50-case report. Interaction work completes push-to-talk capture, transcript, preview, and confirmation UX. The team finishes the 50-transcript plan set and a 20-utterance live speech smoke run. Autonomy work stays on hardware when the M2 gate is open; resolver work pulled forward during idle capacity remains M4 work and does not gate M1. The language exit is earned on sim through live microphone input.
