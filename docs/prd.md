...
- Deliverables: expand the M2.0 selected-feed proof into four-node MediaMTX ingest, WebRTC/MJPEG serving, recording, detection events, and measured latency (Platform); add the live camera mosaic, focus-by-selection, telemetry and sensor state, detector, attention promotion, and operator confirmation in the console (Interaction); use wall-mounted tag36h11 AprilTags, detected through `apriltag-python` or `cv2.aruco`, as the shared indoor localization source. Each periodic tag reacquisition is fused with VPS/IMU dead reckoning between fixes after camera calibration for every camera and imaging configuration used as localization evidence. The Mini 3's controllable gimbal range is -90° to +60°, which does not point far enough upward to reliably view ceiling-mounted tags, so the placement design is wall-only. Prove localization and independent collision-clearance sensing before exposing `map_area`; then preview and confirm it through the operator console, navigate one drone and then two through approved room poses on a supplied occupancy map, partition known room targets, attach every completed capture bundle to the room catalog, and submit the accepted run to per-room World API jobs with `public: false` (Autonomy with Platform and Interaction support).
