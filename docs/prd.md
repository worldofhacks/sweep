...
- Scheduling: language work may begin after M1.E. M1.5 simulator and console work for formation, altitude, spacing, sweep, and the 4-to-6-drone simulator expansion may begin after M1.4. #17 gates its production relay-to-autonomy exit. Hardware safety work takes priority until M2.0 passes. After M2.0, gesture, indoor autonomy, and the third and fourth physical nodes may proceed concurrently; the input lane may land first.
...
- M2.0 safety and evidence: keep the complete arbiter, network stop, state and confirmation checks, live registry, roster and connection epochs, geofence, ceiling, spacing, battery, link-loss and positioning-loss behavior, append-only JSONL audit log, and the section 8.5 flight rule. M1.5 may build and exercise formation, altitude, spacing, and sweep in the simulator after M1.4. Its production relay-to-autonomy exit awaits #17. The M2.0 checkpoint itself exercises its eight named flight-control intents. Altitude gesture, detector, mosaic, language and LLM work, replay UI, metrics dashboard, session report, and release polish follow their own listed gates.
...
- Entry: M2.0 is green. Its one selected live feed provides the narrow media proof. Recording, multi-stream work, detector prototyping, and the post-M1.E language lane may then run concurrently; relay and console integration wait for their shared contracts. `map_area` remains `unsupported` until shared indoor localization and collision-clearance sensing pass the M3 gate.
...
M1.5 provides simulated evidence through confirmed `land_all`. The Appendix E disarm step remains an owner and acceptance hold until it has named scope and production-path evidence.
