...
| `search(zone_id, target_label)` | one configured destination and an explicit object label | a bounded search request and detection events | absent or ambiguous zone or label, stale state, or unsafe route → refuse or hold and alert |
...
Configured navigation and search have separate deployment gates. Current software adds `navigate {zone_id}` and `search {zone_id, target_label}` only when their deployments are configured. `search` requires both fields from the user and refuses an absent or ambiguous destination or label. `map_area` remains the C4 building-level intent.
