...
from tools.ohmni_live_tag_mapper import MAX_ARCHIVE_BYTES, MAX_ARCHIVE_DURATION_S, MAX_ARCHIVE_RECORDS
from tools.ohmni_local_map_candidate import MAX_MANIFEST_BYTES, _archive, _digest, _object, _pin, _read_regular
...
def _boundary_pose(events: Sequence[Observation], first: bool) -> tuple[str, int, list[list[float]]]:
    ...
    poses.sort(key=lambda event: _capture(event))
...
def _read_evidence(
    ...
    calibration_payload = _read_regular(_safe_evidence_path(evidence_root, calibration_pin, "calibration"), 1024 * 1024)
    mount_payload = _read_regular(_safe_evidence_path(evidence_root, mount_pin, "mount"), 1024 * 1024)
    ...
    return calibration, mount, calibration_pin, mount_pin
...
def build(
    ...
    if type(maximum_continuity_gap_ns) is not int or not 0 <= maximum_continuity_gap_ns <= MAX_CONTINUITY_GAP_NS:
    ...
    calibration, mount, calibration_pin, mount_pin = _read_evidence(request, evidence_root)
    ...
    for archive in archives:
        ...
        if previous is not None:
            if start[0] != previous[0] or start[1] < previous[1] or start[1] - previous[1] > maximum_continuity_gap_ns:
                raise ValueError("archive pose continuity time proof failed")
            if math.dist(_translation(previous[2]), _translation(start[2])) > request["maximum_translation_spread_m"]:
    ...
    if len(all_events) > MAX_MULTI_ARCHIVE_OBSERVATIONS or sum(map(len, observation_payloads)) > MAX_AGGREGATE_BYTES:
    ...
    result = fuse_observations(
        ...
        registration=None,
        input_pins={
            "observations": {"path": "inputs/combined-observations.jsonl", "sha256": _digest(b"".join(observation_payloads))},
    ...
    try:
        ...
        result["continuity"] = {"maximum_gap_ns": maximum_continuity_gap_ns, "checked_boundaries": max(0, len(archives) - 1)}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = build(args.archives, args.fusion_request, args.evidence_root, args.output, args.maximum_continuity_gap_ns)
    except (OSError, TypeError, ValueError) as error:
        raise SystemExit(f"ohmni multi-archive tag candidate failed: {error}") from error
    print(json.dumps({"output": str(args.output), "tag_count": len(result["candidates"])}, sort_keys=True))
    return 0
