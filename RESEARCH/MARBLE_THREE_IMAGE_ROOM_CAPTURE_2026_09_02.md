...
The `reconstruct_8` yaw sequence derives its spacing from `yaw_step <= horizontal_fov * (1 - overlap_fraction)`. Forty percent overlap is an initial Sweep experiment. With an 82-degree horizontal field of view, the maximum step is about 49 degrees, so eight headings at 45-degree increments form a sensible first test. A level yaw ring misses the floor and ceiling and cannot satisfy `pano_360`.
