...
def _anthropic_body(request: ModelRequest) -> dict[str, object]:
    return {
        ...
        "system": (
            ...
            "means exactly one ground_velocity step: forward linear_mm_s=180/angular_mrad_s=0/duration_ms=1700, "
            "left linear_mm_s=0/angular_mrad_s=350/duration_ms=250, "
            "right linear_mm_s=0/angular_mrad_s=-350/duration_ms=250. "
            ...
            "never invent a route, and never choose the device selection. "
            "Each destination's enabled_kinds restrict which reviews it supports. "
            "Resolve room names and tag references only using catalog names, IDs and aliases. "
            "A tag alias is an approved arrival destination, not the tag's physical surface. "
            "If a name or alias matches multiple destinations, clarify ambiguous_location; "
            "if no destination matches, clarify unknown_reference. "
            "Named navigation applies to the current ground-only or aircraft-only selection. "
            "If the operator asks for different devices, clarify ambiguous_selection and ask "
            "them to select those devices before requesting the route. Do not silently use "
            "the current selection for a different explicitly named device. "
            "Ground named navigation is supported by navigate review when the catalog is "
            "present; the ground pulse restriction above does not prohibit named reviews. "
            "Survey means coverage without object identification; search needs a supported "
            "target class. Do not replace one with the other.\n"
        ...
        "properties": {
            ...
            "args": {
                ...
                "properties": {
                    ...
                    "duration_ms": {"type": "integer", "minimum": 1, "maximum": 1700},
                    "dx": {"type": "number"},
                    "dy": {"type": "number"},
                    "delta": {"type": "number"},
