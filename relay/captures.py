"""Bounded capture state; the audit log retains the complete history."""
...
from relay.contracts import MAX_CAPTURE_BUNDLE_MEDIA_ITEMS, CaptureBundleFrame, MediaFileRecord

CaptureKey = tuple[int, int, str]
MAX_CAPTURE_ENTRIES = 64
MAX_MEDIA_FILES_PER_CAPTURE = MAX_CAPTURE_BUNDLE_MEDIA_ITEMS
...
class CaptureLedger:
    ...
    def validate_media(self, record: MediaFileRecord, *, t: int) -> None:
        """Check media without mutation, before the transport event identity is claimed."""
        ...
        self._checked_entries(key, entry)
    ...
    def eviction_candidate(self, key: CaptureKey) -> CaptureKey | None:
        """Return the oldest closed entry to evict; refuse if every entry is open."""
    ...
    def _commit(self, key: CaptureKey, entry: CaptureEntry) -> None:
        self._entries = self._checked_entries(key, entry)

    def _checked_entries(
        self, key: CaptureKey, entry: CaptureEntry
    ) -> dict[CaptureKey, CaptureEntry]:
