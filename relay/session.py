...
class RelaySession:
    ...
    def record_navigation_packet(self, packet: Mapping[str, object]) -> dict[str, object]:
        with self._lock, self._audit_operation():
            ...
            connection_epoch = packet.get("connection_epoch")
            if type(drone_id) is not int or type(connection_epoch) is not int:
                raise ValueError("navigation packet has invalid connection identity")
            self.registry.check_current(drone_id, connection_epoch)
            event = {key: value for key, value in packet.items() if key != "signature"}
            self._append_audit(event)
            return dict(packet)
    ...
    def control_localization(self, drone_id: int) -> ControlPose | None:
        """Return the retained diagnostic pose for compatibility with prior callers."""
        return self.control_pose(drone_id)

    def apply_control_localization(self, snapshot: FleetSnapshot) -> FleetSnapshot:
        """Keep the legacy projection hook inert; diagnostic poses cannot affect flight state."""
        return snapshot
