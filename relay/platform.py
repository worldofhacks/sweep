    ...
    def __init__(
        ...
        *,
        ...
        with ExitStack() as cleanup:
            ...
            identities = runtime.settings.device_identities()
            for source in self.observations.sources.values():
                ...
                identity = identities.get(source.drone_id)
                if (
                    source.drone_id not in keys
                    or identity is None
                    or (identity.device_class.value != source.node_type.value)
                ):
                    raise SettingsError(
