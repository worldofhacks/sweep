...
`SWEEP_LOCALIZATION_KEYS_JSON` is an optional, separately generated per-aircraft credential map. It defaults to empty, and every configured relay, adapter, and localization secret must be distinct. Credentials alone do not enable localization projection: the host must also provide measured clock mappings, exact deployment/source pins, and bounded freshness and uncertainty limits. The standalone `relay.app:app` has no projector. `relay.main` loads `SWEEP_CONTROL_LOCALIZATION_CONFIG` and creates a configured projector for each session. A projected pose also requires an explicit per-aircraft adapter key; the demo-only shared relay-token fallback is never used to sign one.
