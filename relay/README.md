...
- Drone IDs are unique positive signed-32-bit integers, with at most 32 per list; configured physical capacity and formation limits remain separate. The current `selection` may be empty; `select.args.ids` may not.
...
The real-fleet launcher requires `SWEEP_ALLOW_SHARED_ADAPTER_TOKEN=false` and distinct device credentials. Shared tokens are confined to isolated tests. The operator configuration supplies measured policy and freshness bounds explicitly; `.env.example` does not supply a qualified motion policy or demonstration roster.

`SWEEP_NODE_TYPES_JSON` is an optional map from configured adapter IDs to `aircraft` or `ground`; absent IDs are aircraft for compatibility. For example, `'{"9":"ground"}'` assigns adapter ID 9 to the ground class. The host configuration is authoritative: a configured ground node must sign `node_type: "ground"` on join, while a legacy join without that field is accepted only for aircraft. Physical aircraft capacity is explicitly configured through `SWEEP_PHYSICAL_AIRCRAFT_LIMIT` (default four, maximum 32); the registry permits up to 32 ground nodes. Capacity does not qualify simultaneous physical operation or relax the separate formation and per-intent bounds. State and membership records expose each node's class and signed adapter capabilities.
