...
Server WebSocket event types are `auth.accepted`, `auth.refused`, `membership`, `state`, `telemetry`, `safety_action`, `acknowledgement`, `refusal`, and the node-authored `capabilities`, `capture_readiness`, and `node_status`; a node's socket additionally receives `command` and `control_heartbeat`. Every one carries `event_id`. `safety_action` is a strict union: node-local link-loss events expose one aircraft and connection epoch, while operator-presence events expose the retry/terminal status and a bounded list of exact aircraft identities. A refusal always includes all of `intent_id`, `command_id`, `drone_id`, `connection_epoch`, `roster_version`, `reason`, and `detail`; context fields are deliberately present as null when they do not apply. Acknowledgements use the same always-present context fields; `command_id` is non-null for adapter facts and nullable for relay/orchestrator intent-level lifecycle events.
...
`SWEEP_OPERATOR_PRESENCE_WATCHDOG_JSON` is required and selects `{"action":"hold"}` or `{"action":"estop"}`. `operator_timeout_ms` must be at least 3000 ms, leaving three opportunities at the one-second activity rate limit. Accepted operator intents and explicit authenticated `operator_presence` frames carrying `activity: "interaction"` refresh relay receipt time; the relay accepts and audits at most one such frame per second.

On expiry with active aircraft, the composition freezes their drone and connection-epoch identities, publishes an operator-presence `safety_action`, and retries a reserved `safety:` stop until the exact targets produce a completed terminal result. Pending or failed attempts suppress relay control heartbeats, allowing each phone's independent watchdog to enter its configured hold/fail-safe path. A target-identity change fails the attempt and is handled by a new frozen retry. Expiry records `not_required`, invalidates operator freshness, and does not latch e-stop only when every current registered, ready, or degraded identity is explicitly reported landed or disarmed. Operator-presence safety events expose `requested`, `retrying`, `awaiting`, `confirmed`, `failed`, or `not_required` status plus up to the simulator ceiling of six exact aircraft identities; node link-loss events retain their existing aircraft-local shape.
...
During bounded orderly shutdown, a composed relay writes `<session-sha256>.report.json` beside that session's JSONL log. The writer accepts only the canonical validated audit snapshot, enforces record/source/output and JSON-structure bounds, and binds the report to the audit digest and final sequence. Command timing accepts only ordered acknowledgements whose intent, aircraft, connection epoch, and roster match the exact command. Raw acknowledgement timestamps are retained; an accepted adapter timestamp that precedes relay issuance is explicitly flagged, and terminal latency is null when that clock inversion makes the subtraction unsound. Reports include operator-presence safety transitions and explicitly say `complete` only after every autonomy worker has stopped; a shutdown deadline produces an `incomplete` report when enough deadline remains to durably write one.

The console sends no background visible-tab heartbeat. A trusted pointer press or key press in the visible document sends the strict interaction frame, coalesced to at most once per second; an unattended visible tab therefore expires. Closing the console or losing the link removes the activity listeners. A renewed interaction does not restart an interrupted plan or bypass a pending safety action.

This is a bounded software/simulator foundation for M2.3, not completion of issue #60. An abrupt process or power loss leaves the canonical JSONL/SQLite audit evidence but cannot generate a post-crash report in this slice. Hardware-backed action evidence and the remaining position-convergence, graded-geofence, tracking-error, and physical-takeover acceptance work remain in #60.
