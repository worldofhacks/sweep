...
A ground vehicle joins, reports telemetry, reaches `ready`, and is projected and labelled, but it is not in the autonomy snapshot yet: `relay/autonomy.py relay_snapshot` excludes every device whose class is not aircraft, because `FleetSnapshot` carries no device class or drive state. Until the planner's per-class device state lands, a ground vehicle in a mixed session cannot be selected or commanded, and a fleet-wide `estop`, `hold`, spacing, or geofence check computed from that snapshot does not include it; its own node watchdog and screen stop remain its safety path.
