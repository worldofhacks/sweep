...
SWEEP_COMMAND_DEADLINE_MS=90000
SWEEP_VIRTUAL_STICK_HZ=10
SWEEP_NODE_WATCHDOG_HOLD_MS=2000
SWEEP_NODE_WATCHDOG_FAILSAFE_MS=10000
```

`SWEEP_ADAPTER_BACKEND` selects which adapters `relay.bridge.build_adapters` and `build_dispatcher` construct for a session: `sim` (the deterministic simulator, with an explicit `SimCameraConfig`) or `remote` (one `RemoteBridgeAdapter` over the bridge wire, bounded by `SWEEP_COMMAND_TTL_MS`). The relay itself never dispatches; `relay.autonomy`, the composition `relay.main` runs, calls that factory for each accepted intent. `SWEEP_COMMAND_DEADLINE_MS` bounds one command's total wait regardless of non-terminal heartbeats and must be at least the TTL; its default exceeds the node's own takeoff and landing timeouts (25 s and 60 s) so a slow auto landing completes instead of being reported as still executing. At the deadline the adapter stops waiting, returns the last non-terminal acknowledgement, and the plan reports `executing`; the relay then keeps the command for one more deadline (the late-acknowledgement window) and settles the intent from the node's late terminal answer, or fails it as `adapter_timeout` when the window passes. `SWEEP_VIRTUAL_STICK_HZ` must stay within the documented 5 to 25, and the watchdog values must satisfy `0 <= hold < failsafe`. These are demo values; measure and configure them for a hardware session.
...
The node answers with the adapter acknowledgement frame above, echoing `intent_id`, `command_id`, `drone_id`, `connection_epoch`, and `roster_version`: `accepted` on admission, `executing` when work starts, then `completed` or `failed`. A failure names one of the machine-readable reasons the node may return, `stale_command`, `out_of_order_command`, `authority_lost`, `watchdog_hold`, and `watchdog_failsafe`, or another snake_case reason. Every acknowledgement is bound to the session's issued-command ledger: `command_id` must name a command this session issued, and its `drone_id`, `connection_epoch`, `intent_id`, and `roster_version` must match that command's; otherwise the relay refuses it with `unknown_command_id` or `command_binding_mismatch` (a second terminal answer with `command_already_terminal`) and audits the refusal with the offending `command_id`. On the `remote` backend every command is relay-issued, so an unknown `command_id` is always refused (`RelayLimits.require_issued_commands`); on the in-process backends an intent with no relay-issued commands may still be acknowledged by an adapter executing planner commands on its own, while an intent that has relay-issued commands is bound to them either way. `RelaySession.await_command_acknowledgement` hands each acknowledgement to the waiting remote adapter and releases the wait after a terminal status or a timeout. A terminal answer that arrives after the wait was released is still accepted inside the late window: it is audited as a `late_acknowledgement` record (command, status, and how late) and settles the intent the autonomy layer left executing (`expect_late_acknowledgement`): `completed` when that command completed the plan, otherwise `failed` with the node's reason or `adapter_timeout`, always under `source: "relay"` with the `accepted_plan` projection cleared. After the window a terminal answer is refused as `late_acknowledgement_expired` and the intent has already failed as `adapter_timeout`.
...
`relay.autonomy` is the planner/arbiter consumer behind `create_app`'s `intent_sink_factory` and `leave_authorizer_factory`; `relay.main` builds it with `create_autonomy_app`. It implements the session's sink contract: the factory receives the `RelaySession` and keys one `AutonomySession` by its session id, the session calls the sink once per accepted intent outside its own lock, and the sink returns an `IntentSinkResult` the session applies inside that intent's operation (the `autonomy_result` audit record, `armed_update` and `selection_update` through `update_control_projection`, `estop_update`, and the lifecycle under `source: "autonomy"` with a null `command_id`; node acknowledgements keep `source: "adapter"` and their `command_id`). The sink hands the intent to one of three worker lanes (below) and blocks until that lane has a result; the session's own execution lock serialises operator intents while `hold` (`concurrent_intents`) and `estop` bypass it so they can preempt a running plan. A worker builds the `FleetSnapshot` from `current_state()`, calls `relay.bridge.build_dispatcher` for that snapshot, and runs `planner.controller.AutonomyController` (capability gate, `check_intent`, plan, `check_plan`, per-command `check_command`, dispatch). Arm and selection apply only once their plan completed and only while the plan's roster is still the session's roster (otherwise the result becomes `invalidated` with `stale_roster`). A plan still waiting on a node's terminal acknowledgement is reported as `executing`: the worker publishes it as `accepted_plan`, registers it with `expect_late_acknowledgement` so the relay settles it on the node's late answer or on the late window passing, and never resumes it itself. Graceful leave is authorized through `planner.roster.authorize_graceful_removal` on the same snapshot. `adapters/sim/app.py` keeps its own composition (`planner.relay_bridge.AutonomyRelayBridge`) on the same contract.
...
On `remote`, every planned command becomes a signed `command` frame to the node bound to that aircraft and the node's acknowledgements complete it. On `sim`, the dispatcher runs the in-process simulator built from the snapshot; a live relay has no telemetry source for it, so the registry stays as the nodes report it and `sim` remains the CI backend. Not wired yet: resuming a plan's remaining commands after a late completion (`resume_after_completion`; the relay settles such an intent as `failed` instead), the positioning-loss monitor (`handle_positioning_loss`), and motion-conflict pairing (`execute_pair`); the physical RC remains the independent stop path. While an intent executes, its audit operation is open, so `GET /session/{session_id}` replay is fenced until it commits; on `remote` that can last up to the command deadline.

The three lanes give stops priority. `estop` runs at once on its own lane and cancels whatever the other two lanes are executing; `hold` runs at once on a second lane and cancels a running operator motion or camera plan (`takeoff`, `translate`, `come_home`, `capture_room`), but queues behind a running `land_all` or `estop` rather than interrupting a safety plan; everything else runs in arrival order on the third lane. Both stops also cancel accepted motion and camera intents that have not started (`admit_intent` registers every accepted intent before it executes). The network stop latches from the intent, never from its plan: the sink sets the session's `estop` before the stop is queued, so nothing is dispatched after a stop was accepted without the latch in place, and marks every later snapshot `estop_active`, so an operator intent that starts in between is refused rather than sent; the result's `estop_update` applies the same latch again inside the intent operation, so no worker, plan, or session-lookup failure can lose it. Cancellation is atomic with the wire: under the session lock, the stop records the cancelled intent as `invalidated` with reason `preempted_by_estop` or `preempted_by_hold`, so `issue_command` refuses anything that plan tries to send afterwards; the plan's dispatch also checks its flag before every command, before every send, and after every acknowledgement wait, and exits without a best-effort hold because the stop is the safety action; the cancelled intent's own sink call then returns nothing because the stop already reported it, and the invalidation records travel with the stop's result. A cancelled plan therefore exits at its next acknowledgement or after at most one command TTL of silence, and every command is bounded in total by `SWEEP_COMMAND_DEADLINE_MS`. `RemoteBridgeAdapter.estop` sends to every aircraft before waiting on any acknowledgement, so a node that stays silent fails only its own result (`adapter_timeout`) while the stop still latches. `relay/tests/test_autonomy_roundtrip.py` runs the M2.0 workflow from console intents through the composition to two fake nodes; `relay/tests/test_autonomy.py` proves the latch precedes any dispatch and refuses the next operator intent as `estop_active`.
