...
Top-level `captures` lists every capture the session has retained, oldest activity first, one entry per aircraft, connection epoch, and capture id: `capture_id`, `drone_id`, `connection_epoch`, `room_id`, `pattern`, `coverage`, `status`, `reason`, `detail`, `files`, and `updated_at`. `files` holds the node's `media_file` records verbatim (a later record for the same `file_id` replaces the earlier one, so a `pending` capture-time record becomes the `completed` retrieval record with the bytes' SHA-256). `room_id`, `pattern`, `coverage`, `status`, `reason`, and `detail` are null until a `capture_bundle` closes the set; the command wire carries only `capture_id`, so the closing bundle is the one the autonomy composition records from the dispatcher's validated result (`source: "autonomy"` in the audit log), or a node-authored `capture_bundle` frame. The relay never invents a room, pattern, or quality for an open capture. Entries survive a rejoin under their own epoch; nothing here is a readiness or safety fact.
...
- `media_file`: the `MediaFile` fields (`capture_id`, `file_id`, `timestamp_ms`, `drone_id`, `connection_epoch`, `pose`, `actual_yaw_deg`, `gimbal_pitch_deg`, `intrinsics`, 64-character lowercase hex `checksum_sha256`, `storage_ref`, `retrieval_status`). `retrieval_status` is `pending` for the record a node sends at capture time, before the bytes have left the aircraft (its `checksum_sha256` is then sixty-four zeros and `storage_ref` names the file on the aircraft), and `completed` for the record sent after `retrieve_media` downloaded the file (SHA-256 of the bytes, `storage_ref` the file on the phone); `failed` and `unsupported` are the other values. A node sends the `media_file` before the terminal acknowledgement of the capture or retrieval command that produced it. Audited, retained for the command wire (`RelaySession.media_files` returns the latest record per file for the current epoch), written as `<file_id>.json` under `SWEEP_SESSION_LOG_DIR/<sha256 of the session id>-captures/drone-<id>/epoch-<n>/<capture_id>/`, and projected into `state.captures`; the frame itself is not fanned out.
- `capture_bundle`: `room_id`, `capture_id`, `pattern`, `coverage`, `status`, nested `media` records, and nullable `reason` and `detail`; a `failed` or `unsupported` bundle requires a machine-readable reason. Audited, retained, written as `bundle.json` beside the media records, and projected into `state.captures`; not fanned out. The autonomy composition records the bundle the dispatcher composes for a `capture_room` plan through the same path, with `source: "autonomy"` added to the audited event; a node may send its own only when it knows the room and pattern.
