...
- `join`: `adapter_id` and a non-empty unique `capabilities` list. The list may carry the device's class as one `class:aircraft` or `class:ground_vehicle` entry; absent means aircraft, so the Android bridge's frames are unchanged. More than one `class:` entry, an unknown class, or a class that differs from the one `SWEEP_DEVICE_CLASSES_JSON` configures for that ID refuses the join with `device_class_mismatch`. Aircraft advertise `flight` (the readiness minimum) and camera patterns `pano_360`, `reconstruct_8`, or namespaced `camera:<pattern>`. Ground vehicles advertise `ground_drive` (their readiness minimum) and optionally `lidar`, `camera`, `neck`, `speech`, `lights`, and `screen`. Capability lists are limited to 64 canonical printable entries, 512 UTF-8 bytes per entry, and 8192 UTF-8 bytes for the canonical JSON list. State-retained identity/profile text is also canonical printable UTF-8 before mutation.
- `readiness`: `connection_epoch`, `home_pose_confirmed`, `control_authority`, and `rc_safety_operator_present`. When confirmed, home XYZ is captured from the current-epoch telemetry already held by the relay, for both classes. For an aircraft `control_authority` is virtual-stick authority and `rc_safety_operator_present` an RC pilot ready to take over. For a ground vehicle `control_authority` means the wheels are enabled and no local override is active, and `rc_safety_operator_present` means a spotter is beside the robot with the on-screen STOP in reach. The readiness gates are `identity_unverified`, `adapter_capabilities_missing`, `flight_capability_missing` (aircraft only), `drive_capability_missing` (ground vehicles only), `telemetry_missing`, `telemetry_stale`, `home_pose_missing`, `control_authority_missing`, and `rc_safety_operator_missing`. An unconfirmed home pose is cleared only while the device is grounded: an aircraft `armed`, `disarmed`, or `landed`, a ground vehicle `docked`, `idle`, or `stopped`.
...
Telemetry extends Appendix B with transport identity and ordering fields. A ground vehicle sends the same keys with `z` and `vz` at `0.0`, its `state` from the drive vocabulary `docked`, `idle`, `moving`, `stopped`, `fault` (`planner.models.DriveState`), and `pos_quality` from its odometry (0.0 without odometry, 0.6 for wheel odometry alone, higher once an external observer corrects it):
...
```text
drone_id, device_class, unit, connection_epoch, membership, readiness_reasons,
flight_state, battery, link, pos_quality, control_authority, last_seen_at,
camera_patterns, selectable, adapter_id, adapter_capabilities, home_pose,
rc_safety_operator_present, telemetry, membership_history,
membership_history_truncated, camera_capabilities, node_status, video, sensor
```

`device_class` is `aircraft` or `ground_vehicle` and `unit` the positive ordinal described under "Run the relay"; both come from configuration at join time and are material to the audit projection. Ground vehicles share the ID space and every other key: `flight_state` carries their drive state string.
...
- `sensor`: a scan from the device, currently only `kind: "lidar_scan"`. `pose` `{x, y, yaw_deg}` is the device's pose at scan time in the same frame as its telemetry, `yaw_deg` in [0, 360) counter-clockwise from +x; scan angle 0 points along the device's forward axis and angles increase counter-clockwise from `angle_min_deg` (in [0, 360)) in steps of `angle_increment_deg`, one of 0.5, 1.0, 2.0. `range_min_m` and `range_max_m` are finite with `0 <= min < max`. `ranges_cm` holds integers 0 through 65535 (0 means no return), exactly `360 / angle_increment_deg` entries (720 at most), and the frame's canonical JSON is at most 8192 UTF-8 bytes. The relay accepts at most five frames per second per device; a faster frame passes the same parsing and identity checks but is then dropped silently, counted in `/metrics` as `sensor_frames_dropped_total`, without a refusal or a transport claim. An accepted frame is retained as the device's latest scan (`RelaySession.latest_sensor`, and every callable in `RelaySession.sensor_listeners` is called with it after the operation commits), fanned out unchanged to consoles as a `sensor` event, reflected in the drone's `sensor.last_scan_at`, and folded into the session's occupancy grid (see "Occupancy map"); it is not appended to the audit in full (see "Log volume").
...
Appendix B carries no physical armed, physical-RC, storage, camera-readiness, active-task, position-loss, or Sweep-operator facts, so `relay_snapshot` asserts them explicitly and fails closed: operator presence and activity come from the latest accepted console or keyboard intent in the session (the arbiter's operator timeout bounds it; no intent yet means no operator); physical armed evidence is derived from the authoritative telemetry flight state exactly as the simulator reports it (every state except `disarmed` and `landed`), so the arbiter's physical-armed gate cannot refuse anything its flight-state gates do not already refuse, an accepted limitation until a node reports a motor-armed fact; physical-RC availability is the same signed `rc_safety_operator_present` readiness claim, because the wire carries no separate RC-link fact, so the arbiter's two RC gates are intentionally collapsed into one at this stage and must not be read as defence in depth; storage comes from the node's current-epoch `capabilities` frame (none means zero) and camera readiness requires the node's latest current-epoch `capture_readiness` frame to report both `camera_ok` and `storage_ok` (no frame means not ready); `active_task_id` is null because intents run one at a time; `position_loss_since_ms` is null so the controller's dwell falls back to the position timestamp. Aircraft without current-epoch telemetry are excluded from commands and mark the fleet observation incomplete; C2 `disarm` therefore cannot withdraw the session-wide arm authorization until every registered aircraft has current proof that it is physically disarmed.

On `remote`, every planned command becomes a signed `command` frame to the node bound to that device and the node's acknowledgements complete it. If a nonterminal result is followed by a timeout, the exact issued-command ledger retains the execution owner for a bounded interval and a later terminal result resumes the plan without resending the completed command. On `sim`, the dispatcher runs the in-process simulator built from the snapshot; a live relay has no telemetry source for it, so the registry stays as the nodes report it and `sim` remains the CI backend. Motion-conflict pairing (`execute_pair`) is not wired; the physical RC remains the independent aircraft stop path.

The periodic autonomy monitor holds the eligible fleet when position quality falls below the safety threshold or position evidence becomes stale. It runs for an armed session, physically airborne aircraft, or a robot reporting motion; unarmed idle robots do not trigger it. One loss episode queues one HOLD, with fresh stops after roster or epoch changes. Continuing poor-quality telemetry preserves the original loss time. Remote fleet stops are all sent before waiting for acknowledgements, so one silent device cannot prevent its peers from stopping. Pending acknowledgements remain pending evidence for at most `max(command_deadline_ms, command_ttl_ms)` after collection returns. On expiry, the relay records `adapter_timeout` and rejects later acknowledgements for the retired commands; it does not claim those stops completed. After `positioning_loss_hold_ms`, it queues an aircraft-only LAND_ALL once the fleet HOLD completes, fails, or expires. Normal motion stays blocked while the loss or its queued, running, or pending safety response remains active, including when telemetry recovers during adapter I/O. Both phases use the normal worker lanes, preemption, signed commands, audit ledger, and acknowledgement resumption. They neither refresh operator presence nor change the operator's selection. Ground-only sessions hold without landing. Ground `fault` and `docked` states are not mobile: an Ohmni node must stop locally before reporting an odometry fault, and its heartbeat watchdog and screen STOP remain independent stop paths.
...
- An accepted `sensor` frame is never written in full. At most once per `SWEEP_AUDIT_STATE_INTERVAL_MS` per device the audit receives a `sensor_digest` record carrying the frame's envelope (`v`, `t`, `event_id`, `session`), `drone_id`, `connection_epoch`, `kind`, `count` (entries), `valid` (non-zero entries), and `min_cm` and `max_cm` over the valid entries (null when none). A scan changes only the volatile `sensor.last_scan_at`, so it never triggers a `state` record; scans are a display feed, not decision evidence.
...
`GET /api/sessions/{id}/map` returns the session's occupancy grid as an 8-bit grayscale PNG: 0 occupied, 255 free, 128 unknown. The relay keeps one grid per session, 0.05 m per cell, covering the safety geofence expanded by 2.0 m on each side, or a fixed square from -10 m to +10 m on both axes when the process runs without `SWEEP_SAFETY_JSON`. Every accepted `sensor` frame updates it: each valid range casts a Bresenham ray from the scan's `pose`, the cells the ray crosses lose log odds and the cell it ends in gains them, so a cell is confident only where scans agree. A zero range is no return and is skipped entirely, a range below `range_min_m` is skipped as untrustworthy, and a range beyond `range_max_m` clears its ray out to the maximum without marking an obstacle; a scan whose pose falls outside the grid changes nothing.

`X-Sweep-Map-Resolution-M`, `X-Sweep-Map-Origin-X` and `X-Sweep-Map-Origin-Y` (the world position of the bottom-left cell corner), `X-Sweep-Map-Width`, `X-Sweep-Map-Height`, and `X-Sweep-Map-Updated-At` (relay milliseconds, 0 before the first scan) place the image. Row 0 of the image is the maximum y, so a top-down canvas draws it without flipping. A session that has received no scan has no grid and answers 404. `POST /api/sessions/{id}/map/reset` clears the grid in place, leaving the session answering with an all-unknown image, and writes a log-only `map_reset` record carrying whether there was a grid to clear, so a map that suddenly forgets a wall is distinguishable from a sensor that stopped reporting one.

The grid lives in memory only. It is a display feed: no planner, arbiter, or state projection reads it, nothing restores it after a restart, and it is never written to the audit as pixels.
