...
On expiry with active aircraft, the composition freezes their drone and connection-epoch identities, publishes an operator-presence `safety_action`, and retries a reserved `safety:` stop until the exact targets produce a completed terminal result. Pending or failed attempts suppress relay control heartbeats, allowing each phone's independent watchdog to enter its configured hold/fail-safe path. A target-identity change fails the attempt and is handled by a new frozen retry. Expiry while there are no ready/degraded airborne aircraft records `not_required`, invalidates operator freshness, and does not latch e-stop. Operator-presence safety events expose `requested`, `retrying`, `awaiting`, `confirmed`, `failed`, or `not_required` status plus the exact affected identities; node link-loss events retain their existing aircraft-local shape.
