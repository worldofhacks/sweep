...
def test_media_is_projected_and_a_node_bundle_is_refused_without_authoritative_labels(
    ...
    pending = relay_session.process_frame(
        media_file_payload(
            event_id="media-1", retrieval_status="pending", checksum_sha256="0" * 64
        ),
        adapter_principal,
    )
...
def test_node_bundles_cannot_bypass_media_or_room_authority(
    relay_session: RelaySession, adapter_principal: Principal
) -> None:
    _join(relay_session, adapter_principal)
    for batch in range(2):
        events = relay_session.process_frame(
            capture_bundle_payload(
                event_id="unauthorized-bundle",
                pattern="reconstruct_8",
                media=[media_record(file_id=f"frame-{batch}")],
            ),
            adapter_principal,
        )
        assert events[0]["reason"] == "source_not_allowed"
    assert relay_session.media_files(1, "capture-1") == ()
    assert relay_session.captures() == []
    events = relay_session.process_frame(
        media_file_payload(event_id="unauthorized-bundle"), adapter_principal
    )
...
def test_repeated_media_file_frames_enforce_the_same_cumulative_ceiling_before_claim(
    ...
    assert refused[0]["reason"] == "capture_limit_exceeded"
...
def test_retained_media_capture_keys_are_session_bounded_before_event_claim(
    ...
    monkeypatch.setattr(relay_session._captures, "_max_entries", 2)
    _join(relay_session, adapter_principal)
...
def test_media_batch_rolls_back_exactly_when_audit_append_fails(
    relay_session: RelaySession,
    adapter_principal: Principal,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    before_projection = relay_session.captures()
    ...
    with pytest.raises(AuditLogError, match="disk full"):
        with relay_session._lock, relay_session._audit_operation():
            for index in (0, 2):
                events = relay_session.process_frame(
                    media_file_payload(event_id=f"after-{index}", file_id=f"frame-{index}"),
                    adapter_principal,
                )
                assert events[0]["type"] == "state"
    assert relay_session.media_files(1, "capture-1") == before
    assert relay_session.captures() == before_projection
...
def test_the_composed_capture_bundle_closes_the_capture_from_the_autonomy_result(
    ...
    accepted = relay_session.process_frame(capture_intent, console_principal)
    ...
    events = relay_session.record_execution_result(intent, result)
...
def test_a_refused_media_frame_leaves_the_capture_ledger_untouched(
    ...
    _join(relay_session, adapter_principal)
    relay_session.process_frame(media_file_payload(event_id="media-1"), adapter_principal)

    stale = relay_session.process_frame(
        media_file_payload(event_id="media-2", connection_epoch=2), adapter_principal
    )

    assert stale[0]["type"] == "refusal"
