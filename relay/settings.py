...
class RelaySettings:
    ...
    def __post_init__(self) -> None:
        ...
        if self.perception_key is not None and (
            type(self.perception_key) is not bytes or not 32 <= len(self.perception_key) <= 4_096
        ):
            raise SettingsError("SWEEP_PERCEPTION_KEY must contain 32 through 4096 bytes")
        credentials = [
            self.relay_token,
            *adapter_keys.values(),
            *localization_keys.values(),
            *(() if self.perception_key is None else (self.perception_key,)),
        ]
