...
Forward wheel travel decreases the left encoder count and increases the right count. `SWEEP_WHEEL_DIAMETER_MM` sets the odometry distance scale and defaults to 150.5 mm. Units 11 and 12 use 152.4 mm from the operator’s approximate three-inch wheel radius; this is a nominal scale, with distance accuracy still requiring a measured travel check.
