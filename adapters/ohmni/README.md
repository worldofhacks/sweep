...
`SWEEP_WHEEL_DIAMETER_MM` selects the runtime odometry model; its default is 150.5 mm. The current supervised calibration profiles use 152.4 mm, so set this value explicitly to 152.4 on a node using those calibration results. Matching the recorded model preserves the fitted transform; this setting does not establish the wheel’s precise physical diameter.
