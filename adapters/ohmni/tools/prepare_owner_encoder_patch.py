...
_API = b"  this._api = new LocalApi(this, config_path);\r\n"
_API_REPLACEMENT = _API + (
...
def prepare(source: bytes, expected_sha256: str = REFERENCE_SHA256) -> bytes:
    ...
    if source.count(_REQUIRE) != 1 or source.count(_API) != 1:
        raise ValueError("vendor telebot_node.js does not contain the reviewed patch anchors")
    patched = source.replace(_REQUIRE, _REQUIRE_REPLACEMENT).replace(_API, _API_REPLACEMENT)
    if (
