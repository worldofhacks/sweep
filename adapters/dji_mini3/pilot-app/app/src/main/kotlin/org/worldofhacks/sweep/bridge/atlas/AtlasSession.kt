...
data class AtlasSession(val id: String, val baseUrl: String, val workspace: String,
    ...
    fun api(path: String): HttpUrl {
        ...
        val memory = path.matches(Regex("/atlas/spaces/[a-zA-Z0-9_-]{1,64}/captures/[a-zA-Z0-9_-]{1,64}/memory(?:/(?:inspect|analyze|cancel|review|history(?:/[0-9]{1,3})?|assets/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/media))?"))
        val timeline = path.matches(Regex("/atlas/spaces/[a-zA-Z0-9_-]{1,64}/(?:timeline|captures/[a-zA-Z0-9_-]{1,64}/date)"))
        require(timeline || memory || draftPublish || path.matches(Regex("/atlas/spaces(?:/[a-zA-Z0-9_-]{1,64}(?:/(?:status|invitation|requests|presence|leave|reconstruction|surface-requests|surface-requests/[0-9a-f-]{36}/[0-9a-f]{16}/dismiss|captures/[a-zA-Z0-9_-]{1,64}/media|reconstruction/[a-zA-Z0-9_-]{1,64}/(?:cloud\\.glb|manifest\\.json)))?)?"))) {
