...
/** Typed `args` per operation, matching the `SwarmAdapter` and `CameraCapture` signatures. */
sealed interface CommandArgs {
    val operation: CommandOperation

    fun toJson(): JsonObject

    data class Takeoff(val z: Double) : CommandArgs {
        override val operation get() = CommandOperation.TAKEOFF
        override fun toJson() = Json.json("z" to z)
    }

    data class Goto(val x: Double, val y: Double, val z: Double, val speed: Double) : CommandArgs {
        override val operation get() = CommandOperation.GOTO
        override fun toJson() = Json.json("x" to x, "y" to y, "z" to z, "speed" to speed)
    }

    data class RotateTo(val yaw: Double, val speed: Double) : CommandArgs {
        override val operation get() = CommandOperation.ROTATE_TO
        override fun toJson() = Json.json("yaw" to yaw, "speed" to speed)
    }
    ...
    data class SetGimbalPitch(val pitch: Double) : CommandArgs {
        override val operation get() = CommandOperation.SET_GIMBAL_PITCH
        override fun toJson() = Json.json("pitch" to pitch)
    }
    ...
    companion object {
        ...
        fun parse(operation: CommandOperation, args: JsonObject): CommandArgs {
            val expected = when (operation) {
                CommandOperation.TAKEOFF -> setOf("z")
                CommandOperation.GOTO -> setOf("x", "y", "z", "speed")
                CommandOperation.ROTATE_TO -> setOf("yaw", "speed")
                CommandOperation.SET_GIMBAL_PITCH -> setOf("pitch")
            ...
            if (args.keys != expected) throw ContractError(CODE, "args for ${operation.wire} must be exactly $expected")
            fun number(field: String) = Fields.finiteNumber(args[field], "args.$field", CODE)
            fun text(field: String) = Fields.nonEmptyString(args[field], "args.$field", CODE)
            return when (operation) {
                CommandOperation.TAKEOFF -> Takeoff(number("z"))
                CommandOperation.GOTO -> Goto(number("x"), number("y"), number("z"), number("speed"))
                CommandOperation.ROTATE_TO -> RotateTo(number("yaw"), number("speed"))
                ...
                CommandOperation.SET_GIMBAL_PITCH -> SetGimbalPitch(number("pitch"))
                CommandOperation.CAMERA_READY -> CameraReady
                CommandOperation.CAPTURE_PANORAMA -> CapturePanorama(text("capture_id"))
                CommandOperation.CAPTURE_PHOTO -> CapturePhoto(text("capture_id"))
                CommandOperation.RETRIEVE_MEDIA -> RetrieveMedia(text("file_id"))
            }
        }
    }
}
...
/**
 * Relay-to-node command frame (Phase A spec). `seq` is monotonic per node per connection
 * epoch, `issued_at` and `ttl_ms` are relay-clock milliseconds, and `signature` is the
 * relay's HMAC-SHA256 over the canonical JSON of every other field so the node can verify it.
 *
 * [rawArgs] keeps the `args` object exactly as received so verification re-encodes the
 * bytes that were signed even when the relay formatted a number differently than the typed
 * args would.
 */
    ...
    companion object {
        ...
        fun parse(json: JsonObject): CommandFrame {
            ...
            return CommandFrame(
                ...
                seq = Fields.nonNegativeInt(json["seq"], "seq", CODE),
