...
# Isolated integration testing only. Requires a separate explicit environment,
# SWEEP_ALLOW_TEST_ADAPTERS=true, test-only keys, and a test relay/session.
test-fake-node env_file drone_id session relay:
    uv run --env-file '{{env_file}}' python -m adapters.dji_mini3.fake_node --test-only --drone-id {{drone_id}} --session '{{session}}' --relay '{{relay}}'
