...
def dense_mesh(store: AtlasStore, job_id: str, images: Path, model_path: Path,
               working: Path, output: Path, engine_directory: Path) -> dict:
    import pycolmap
    ...
    for name, path in binaries.items():
        ...
        result = subprocess.run([str(path), "--help"], cwd=working,
    ...
    def stage(name, arguments, status, progress, detail):
        ...
        subprocess.run([str(binaries[name]), *arguments, "--max-threads", "4"],
                       cwd=working, check=True, timeout=900)

    store.progress_reconstruction(job_id, status="densifying", progress=70,
                                  detail="Correcting lens distortion for surface reconstruction.")
    undistorted = working / "dense"
    pycolmap.undistort_images(
        undistorted, model_path, images,
        undistort_options=pycolmap.UndistortCameraOptions(max_image_size=1600), num_threads=4,
    )
    stage("InterfaceCOLMAP", ["-i", "dense", "-o", "scene.mvs", "--image-folder",
                             str(undistorted / "images")], "densifying", 72,
          "Connecting calibrated camera views to the surface solver.")
    stage("DensifyPointCloud", [
        "-i", "scene.mvs", "-o", "scene_dense.mvs", "--resolution-level", "0",
        "--max-resolution", "1280", "--number-views", "5", "--number-views-fuse", "3",
        "--geometric-iters", "2", "--postprocess-dmaps", "1", "--tower-mode", "0",
        "--estimate-roi", "0", "--crop-to-roi", "0",
    ], "densifying", 76, "Estimating surface depth from agreement between overlapping views.")
    ...
    stage("ReconstructMesh", [
        "-i", "scene_dense.mvs", "-p", "scene_dense.ply", "-o", "scene_mesh.mvs",
        "--target-face-num", "100000", "--remove-spurious", "4", "--close-holes", "0",
        "--smooth", "0", "--crop-to-roi", "0",
    ], "meshing", 88, "Reconstructing a bounded surface without automatic hole filling.")
    stage("TextureMesh", [
        "-i", "scene_dense.mvs", "-m", "scene_mesh.ply", "-o", "atlas_mesh.mvs",
        "--export-type", "glb", "--max-texture-size", "2048", "--close-holes", "0",
        "--sharpness-weight", "0", "--empty-color", "8421504",
    ...
    ], "texturing", 95, "Projecting source photographs onto the reconstructed surface.")
    stats = pack_openmvs_glb(working / "atlas_mesh.glb", output / "cloud.glb")
    return {
        **stats, "dense_points": dense_points, "representation": "textured_mesh",
        "engine": f"COLMAP {pycolmap.__version__} / OpenMVS 2.4.0", "dense_engines": engines,
        "experimental": True, "hole_filling": False,
        "dense_settings": {"max_resolution": 1280, "agreement_views": 3,
                           "target_faces": 100000, "max_texture_size": 2048,
                           "global_seam_leveling": False, "local_seam_leveling": False,
                           "tower_mode": 0, "depth_postprocess": "remove_speckles_only"},
    }
